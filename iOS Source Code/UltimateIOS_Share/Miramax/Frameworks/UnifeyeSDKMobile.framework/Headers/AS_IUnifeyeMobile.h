...
#include "AS_ISensorsComponent.h"
...
namespace metaio
{
	...
	*/
	enum ERENDER_SYSTEM
	{
		...
		/// OpenGL ES 1.1 based rendering - not supported in SDK release 3.0 / removed from SDK 3.0 pro 
	...
	*/
	...
	public:
		...
		*
		* \param trackingDataFile Fully qualified path of the XML file that should be loaded. If a ZIP is specified mobileSDK will first try to load "TrackingData_ML3D.xml" from the ZIP
		*		 and if that isn't successfull it'll try to load any other xml from the ZIP.
		...
		*/
		...
		/**
		* \brief Set an image from memory
		*
		...
		*
		*	Note: This function will not work, when running fully free version. It will only work the a signature with 
		*	the Custom Branding Option has been provided.
		*
		...
		*/
		...
		/**
		* \brief Start capturing on a camera.
		*
		...
		* \sa stopCamera
		*/
		virtual Vector2di startCamera( int index, unsigned int width=320, unsigned int height=240 ) = 0;
		
		/**
		* \brief DEPRECATED! Start capturing on a camera. DEPRECATED!
		*
		*  Activate the given camera and start capturing frames.
		*
		* \param index The index of the camera to start (zero-based).
		...
		* \sa startCamera
		*/
		#define activateCamera startCamera
		

		/**
		* \brief Stop capturing on a camera.
		*
		...
		*/
		...
		/**
		* \brief Request a callback that delivers the camera image.
		*
		*	The image will have the dimensions of the current capture resolution.
        *	It will be delivered through the IUnifeyeMobileCallback.onNewCameraFrame
		*
		*	Note: you need to copy the ImageStuct::buffer, if you need it for later.
		*   Note: if both width and height are 0 (default), the last captured image is returned
		*
		...
		*/
		...
		/**
		* \brief Request a high resolution camera image that will be saved as file
		*
		* The result is notified in IUnifeyeMobileCallback. In case of failure, 
		...
		*	You also need to make sure to call this function in the OpenGL thread, e.g. right after mobileSDK->render()
		*
		...
		* \param isBackgroundGeometry True if this geometry should get rendered behind all other geometries.
		* \return Pointer to the geometry. Null pointer if not successful.
		*/
		virtual metaio::IUnifeyeMobileGeometry* loadGeometry( const std::string& geometryFile, bool isBackgroundGeometry = false ) = 0;
		...
		*/
		...
		/**
		* \brief Register (or de-register) sensors component.
		...
		*/
		...
		/**
		* \brief get information about the Sensor, depending on the input command
		*
		* The function passes the command string further until the configured sensor is reached,
		* a sensor function then interprets the command and returns values in string format which 
		* are passed back.
		* This is done to be able to get Sensor Data from an IUnifeyeMobile object without changing the interface.
		*
		* \param command a string
		* \return values in some string format depending on input command
		*/
		virtual std::string getSensorInformation(std::string command) = 0 ;
};
