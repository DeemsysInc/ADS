...
class METAIO_DLL_API GestureHandler 
{
    ...
    std::vector<GeometryExtension*> m_geometryExt;
    ...
    // specify if the geometries can be picked one by one
    bool m_pickable;
...
public:
    ...
    /**
     * Test if the geometries are pickable one by one.
     * \return True if geometries are pickable one by one, false otherwise.
     ...
     * \param pickable Specify if the geometry is pickable one by one.
	 * \sa removeObject, removeObjects
	 * \sa getAllObjects
     */
    void addObject(metaio::IGeometry* geometry, int group, bool pickable);
	 ...
	 * \sa addObject
	 */
	std::vector<metaio::IGeometry*> getAllObjects();
