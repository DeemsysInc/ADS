...
public:
	...
	*/
	...
	/**
	*  Load a tracking configuration from an XML file.
	*
	...
	*/
	virtual bool setTrackingConfiguration(const std::string& trackingConfig, bool readFromFile = true) = 0;
	 ...
	 */
	virtual void startInstantTracking(const std::string& trackingMode,
	                                  const std::string& outFile = "") = 0;


	/**
	 *  Load a camera parameters (calibration) from an XML file.
	 *
	 ...
	 * \return True if successful, false otherwise.
	 */
	virtual bool setCameraParameters(const std::string& calibrationFile) = 0;

	/**
     *  Get the currently used camera calibration (intrinsic parameters).
     *
     * \param[out] imageResolution   The resolution (width and height) of the camera image in pixels.
     * \param[out] focalLengths      The horizontal and vertical focal lengths of the camera in pixels.
     * \param[out] principalPoint    The principal point of the camera in pixels.
     */
    virtual void getCameraParameters(metaio::Vector2di* const imageResolution, metaio::Vector2d* const focalLengths, metaio::Vector2d* const principalPoint) = 0;
	 ...
	 */
	virtual Vector2di setImage(const std::string& source) = 0;

	/**
	 *  Set an image from memory
	 *
	 *	This method is used to set the image source for display and (optical) tracking. Setting a new image will
	 *	include the tracking step. You do not need to call render() afterwards to perform the tracking. Valid
	 ...
	 * \sa resizeRenderer
	 */
	virtual Vector2di getRenderSize() = 0;
	 ...
	 */
	virtual ESCREEN_ROTATION getScreenRotation() = 0;
	 ...
	 * \param width Desired width of the camera image
	 * \param height Desired height of the camera image
	 *
	 * \sa registerCallback to register a callback.
	 */
	virtual void requestCameraImage(const std::string& filepath, int width, int height) = 0;
	 ...
	 * \sa setRendererFrameBuffers
	 */
	virtual void requestScreenshot(const std::string& filepath, unsigned int frameBuffer = 0, unsigned int renderBuffer = 0) = 0;
	 ...
	 */
	virtual float getRenderingFrameRate() = 0;
	...
	*/
	 ...
	 */
	virtual std::vector<metaio::TrackingValues> getTrackingValues(bool rotate = false) = 0;
	...
	*/
	 ...
	 */
	virtual bool getFreezeTracking() = 0;
	 ...
	 * You also need to make sure to call this function in the OpenGL thread,
	 * e.g. right after IMetaioSDK::render()
	 *
	 * \deprecated Use requestScreenshot
	 *
	 ...
	 */
	virtual IGeometry* createGeometry(const std::string& filepath) = 0;
	 ...
	 * \return Pointer to the geometry. Null pointer if not successful.
	 */
	virtual IGeometry* createGeometryFromImage(const std::string& filepath,
	 ...
	 * \return Pointer to the geometry. Null pointer if not successful.
	 */
	virtual IGeometry* createGeometryFromMovie(const std::string& filepath,
	 ...
	 */
	virtual std::vector<metaio::IGeometry*> getLoadedGeometries() = 0;
	 ...
	 *
	 * Note: There must be a model loaded for the specific coordinateSystemID, otherwise the function
	 * will only return (0,0,0)
	 *
	 * \param coordinateSystemID The (one-based) index of the coordinate system in which the 3D point is defined.
	 ...
	 */
	virtual metaio::IBillboardGroup* createBillboardGroup(float nearValue, float farValue) = 0;
	 ...
	 */
	virtual metaio::IGeometry* loadImageBillboard(const std::string& texturePath) = 0;
	 ...
	 * \return A pointer to the created billboard geometry.
	 */
	virtual metaio::IGeometry* loadImageBillboard(const std::string& textureName,
	 ...
	 */
	virtual metaio::ISensorsComponent* getRegisteredSensorsComponent() = 0;
	 ...
	 */
	virtual std::string sensorCommand(const std::string& command, const std::string& parameter = "") = 0 ;
	 ...
	 */
	virtual std::string getCoordinateSystemName(int coordinateSystemID) = 0;
	 ...
	 */
	virtual int getCoordinateSystemID(const std::string& name) = 0;
