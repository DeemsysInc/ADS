...
#define SDK_LICENSE "qwHRF25R0a2F4zq5zL04c3x7rVQSX4crJWfTZZH67Og="
//#define SDK_LICENSE "MFOm8L/icMNWYwkLa9kXbE4Dfn+OTDirKvdk0LO4sBs="
...
CGRect screenCustomBound;
CGSize screenCustomSize;
CGFloat screenCustomWidth;
CGFloat screenCustomHeight;
...
- (void) viewDidLoad
{
    ...
    screenCustomBound = [[UIScreen mainScreen] bounds];
    screenCustomSize = screenCustomBound.size;
    screenCustomWidth = screenCustomSize.width;
    screenCustomHeight = screenCustomSize.height;
    
    if (screenCustomHeight==568){
        CGRect glViewFrame = [self.glView frame];
        glViewFrame.size.height = 568;  // change the location
        glViewFrame.origin.y = 0;  // change the location
...
{
    NSLog(@"onScreenshot %@", filepath);
    //self.testingImageView.image = [UIImage imageWithContentsOfFile:filepath];
}
...
////- (void) onVisualSearchResult(const stlcompat::Vector< metaio::VisualSearchResponse > &response, int errorCode)
//{
//
//}
//
