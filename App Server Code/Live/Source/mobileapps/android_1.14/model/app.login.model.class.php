...
class lAppModel extends SQLQuery {
		...
		try{
			...
			if($vArray['regThrough']!=0){
				if($vArray['uname'] !="")
					...
					$query .= " user_username='".$vArray['email_id']."' OR user_email_id='".$vArray['email_id']."' AND user_register_through=".$vArray['regThrough']." ";
			} else {
				$query .= " user_username='".$vArray['uname']."' AND user_password='".md5($vArray['pwd'])."'";
			}
			
			$query .= " AND user_status =1  LIMIT 1";
	...
	function checkAppUserRegisteredByName($pArray){
		try{
			$query = "SELECT * FROM users as `u`, `user_details` AS `ud` WHERE `u`.user_firstname='".$pArray['user_firstname']."' AND `u`.user_lastname='".$pArray['user_lastname']."' AND `ud`.`user_details_fbid` = '".$pArray['user_details_fbid']."' LIMIT 1";
			$result = $this->selectQuery($query);
			return $result;			
		}
		catch ( Exception $e ) {
	...
	function getUserDetailsById($pUId){
		$query = "SELECT * FROM `user_details` AS `ud` WHERE   `ud`.`user_id` = '$pUId' ";
		$result = $this->selectQueryForAssoc($query);
		return $result;	
	}
