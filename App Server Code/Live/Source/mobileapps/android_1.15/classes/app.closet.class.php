	...
	public function modGetClosetProductValues($cValsArray){
		try{
			...
			for($i=0;$i<count($outResults);$i++)
			{
				...
				if(isset($outResults[$i]["tapForDetailsImgs"]) && $outResults[$i]["tapForDetailsImgs"] != "null") {
						$arrForVideoPdImgs = json_decode($outResults[$i]["tapForDetailsImgs"]);
						
						for($f=0;$f<count($arrForVideoPdImgs);$f++){							
							if($arrForVideoPdImgs[$f]->video!=""){
				 				$outResults[$i]["tapForDetailsImgs"] = "http://".$_SERVER['HTTP_HOST']."/files/clients/".$clientId."/additional/".$arrForVideoPdImgs[$f]->video;
								$outArrClientProduct[$i]["financialVideoType"] ="video";
							}else if($arrForVideoPdImgs[$f]->audio!=""){								
				 				$outResults[$i]["tapForDetailsImgs"] = "http://".$_SERVER['HTTP_HOST']."/files/clients/".$clientId."/additional/".$arrForVideoPdImgs[$f]->audio;
								$outResults[$i]["financialVideoType"] ="audio";
							}
						}
				}
