...
class cMyOffers{
	...
	public function modGetMyOffersList($loggedInUserId, $offerId, $flag){
		try{
			...
			{
				$outArrUserOffers[$i]["offer_image"] = "http://".$_SERVER['HTTP_HOST']."/files/clients/".$outArrUserOffers[$i]["client_id"]."/products/".$outArrUserOffers[$i]["offer_image"];
				if(isset($outArrUserOffers[$i]["offer_back_image"]) && $outArrUserOffers[$i]["offer_back_image"] !="null" ){
				 		$outArrUserOffers[$i]["offer_back_image"] = "http://".$_SERVER['HTTP_HOST']."/files/clients/".$outArrUserOffers[$i]["client_id"]."/products/".$outArrUserOffers[$i]["offer_back_image"];
					} 
				$outArrUserOffers[$i]["offer_valid_from"] = date("m/d/Y", strtotime($outArrUserOffers[$i]["offer_valid_from"]));
				$outArrUserOffers[$i]["offer_valid_to"] = date("m/d/Y", strtotime($outArrUserOffers[$i]["offer_valid_to"]));
	...
	public function modDeleteMyOffersWithUserId($loggedInUserId, $checkFlag,$offerId){
		try{
			global $config;	
			$outResults = array();
		...
		try{
			...
			if($clientId != ""){
				...
				for($i=0;$i<count($outArrClientOffer);$i++)
				{
					...
					if(isset($outArrClientOffer[$i]["offer_back_image"]) && $outArrClientOffer[$i]["offer_back_image"] != "null"){
				 		$outArrClientOffer[$i]["offer_back_image"] = "http://".$_SERVER['HTTP_HOST']."/files/clients/".$clientId."/products/".$outArrClientOffer[$i]["offer_back_image"];
					} 					
					$audio="";
					$video="";
