...
class mMyOffers extends SQLQuery {
	...
	function getClientWithOffersAndInfoDetails($clientId, $loggedInUserId, $offerId)
	{
		...
		of.offer_is_calendar_based,of.offer_is_sharable, oi.offer_info_event_start, oi.offer_info_event_end, oi.offer_info_event_allday, 
	...
	function getClientWithRelatedOffers($clientId, $offerId, $layoutType)
	{
		...
		pd.client_id,  oai.offer_additional_id AS tapForDetailsImgId, oai.offer_additional_media AS tapForDetailsImgs, 
		oai.offer_id AS tapForDetailsImgPdId,cl.id AS clientId, cl.name, cl.background_color, cl.light_color, cl.dark_color,cl.client_vertical_id, cl.is_location_based,`ofr`.relatedto_id, 
		...
		CASE WHEN of.offer_id IS NULL or of.offer_id = '' THEN (SELECT group_concat(`relatedto_offerid` separator ',') as `related_offerid` FROM product_related AS pr WHERE pr.relatedfrom_id= pd.pd_id  AND pr.relatedto_offerid !=0)
		ELSE(SELECT group_concat(`relatedto_id` separator ',') as `related_id` FROM offers_related AS ofr WHERE ofr.relatedfrom_id= of.offer_id AND ofr.relatedto_id != 0) END AS related_offerid,
		CASE WHEN of.offer_id IS NULL or of.offer_id = '' THEN (SELECT group_concat(`relatedto_id` separator ',') as `related_id` FROM product_related AS pr WHERE pr.relatedfrom_id=pd.pd_id AND pr.relatedto_id !=0) ELSE
		(SELECT group_concat(`relatedto_prodid` separator ',') as `related_id` FROM offers_related AS ofr WHERE ofr.relatedfrom_id= of.offer_id AND ofr.relatedto_prodid != 0) END AS related_id,
		...
		of.offer_is_location_based, of.offer_is_calendar_based, odt.offer_discount_value,odt.offer_discount_type,of.offer_is_sharable,
		CASE WHEN of.offer_purchase_url IS NULL or of.offer_purchase_url = '' THEN 'null' ELSE of.offer_purchase_url END AS offer_purchase_url, 			
		...
		LEFT JOIN offers AS of  ON  of.offer_id=`ofr`.relatedto_id AND of.client_id='".$clientId."' AND of.offer_status=1     	
		LEFT JOIN offers_additional AS oai ON oai.offer_id=of.offer_id AND oai.offer_additional_status=1 
...
*/
		...
		if($layoutType!="financial"){
			...
			//$query .= " GROUP BY of.offer_id ASC";
		}
		//$query .= " GROUP BY of.offer_id DESC LIMIT 0,5";
	...
	function getClientAllOffers($clientId,$offerId)
	{
		$query = "SELECT pd.pd_id AS prodId, pd.pd_image AS pdImage, pd.`pd_price` AS pdPrice, pd.`pd_name` AS `prodName`, 
		pd.client_id,  oai.offer_additional_id AS tapForDetailsImgId, oai.offer_additional_media AS tapForDetailsImgs, 
		oai.offer_id AS tapForDetailsImgPdId,cl.id AS clientId, cl.name, cl.background_color, cl.light_color, cl.dark_color,cl.client_vertical_id, cl.is_location_based,`ofr`.relatedto_id, 
		...
		$query .= "
		FROM  offers AS of
		LEFT JOIN offers_related AS `ofr` ON  of.offer_id=`ofr`.relatedfrom_id AND of.client_id='".$clientId."' AND of.offer_status=1     	
		LEFT JOIN offers_additional AS oai ON oai.offer_id=of.offer_id AND oai.offer_additional_status=1 
		LEFT JOIN `offers_info` AS oi ON oi.client_id=of.client_id AND oi.offer_id=of.offer_id 
		LEFT JOIN `offers_discount_type` AS odt ON odt.client_id=of.client_id AND odt.offer_discount_status =1 	AND odt.offer_discount_type_id =of. offer_discount_type_id	
		LEFT JOIN products AS pd ON pd.pd_id=`ofr`.relatedto_prodid AND pd.client_id=`of`.client_id AND pd.pd_status=1 
		LEFT JOIN client AS cl ON (cl.id=of.client_id OR cl.id=pd.client_id ) AND cl.active=1  
		LEFT JOIN `client_details` as `cld` ON `cl`.`id` = `cld`.`client_id` AND `cld`.client_details_status=1  
		LEFT JOIN `countries` as `cs` ON `cld`.`client_details_country_code` = `cs`.`country_code_char2` ";
...
*/
		$query .= "WHERE `of`.client_id=".$clientId;		
		$query .= " AND of.offer_id!=''";
		if($offerId !="0"){
			$query .= " AND of.offer_id  IN (".$offerId.")";
		}
		$query .= " GROUP BY of.offer_id ASC";
