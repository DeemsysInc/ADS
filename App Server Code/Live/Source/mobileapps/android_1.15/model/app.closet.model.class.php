...
class mCloset extends SQLQuery {
	...
	{
		$query = "SELECT c.*,pd.*,cl.name AS client_name, cl.background_color, cl.light_color, cl.dark_color, cl.logo AS client_logo,cl.url AS client_url ,(SELECT group_concat(`relatedto_id` separator ',') as `related_id` FROM product_related AS pr WHERE pr.relatedfrom_id= pd.pd_id ) AS related_id,(SELECT group_concat(`relatedto_offerid` separator ',') as `related_offerid` FROM product_related AS pr WHERE pr.relatedfrom_id= pd.pd_id  AND pr.relatedto_offerid !=0) AS  `related_offerid`,cld.client_details_country_code, cs.country_name, CASE WHEN cs.country_code_char2 IS NULL or cs.country_code_char2 = '' THEN 'US' ELSE cs.country_code_char2 END AS country_code_char2, CASE WHEN cs.country_languages IS NULL or cs.country_languages = '' THEN 'en' ELSE cs.country_languages END AS country_languages, cs.currency_code, IFNULL(pai.pd_additional_id, 'null' )  AS tapForDetailsImgId, IFNULL(pai.pd_additional_images, 'null' ) AS tapForDetailsImgs, 
		IFNULL(pai.pd_id, 'null' ) AS tapForDetailsImgPdId
			 FROM closet AS c 
		LEFT JOIN products AS pd ON pd.pd_id = c.pd_id 
		LEFT JOIN products_addtional_image AS pai ON pai.pd_id=pd.pd_id AND pai.pd_additional_status=1 
		LEFT JOIN client AS cl ON pd.client_id = cl.id LEFT JOIN `client_details` as `cld` ON `cl`.`id` = `cld`.`client_id` AND `cld`.client_details_status=1 
			LEFT JOIN `countries` as `cs` ON `cld`.`client_details_country_code` = `cs`.`country_code_char2` AND `cld`.`client_details_currency_code` = `cs`.`currency_code`
			 WHERE c.closet_status=1 AND c.user_id=".$cValsArray['userId']." AND pd.pd_id!=''";
