...
// 						print_r($arrAdditionalImages);
				...
				if (($arrUser[0]['user_status']!=1) || ($userFBId !="")){
					...
					if($con['user_id'] !=0){
						...
						if ($updateRecord){
							...
							$conDetails['user_id'] = $arrUser[0]['user_id'];	
					...
					if($userFBId!=""){
						$outArrRegisterUser = $this->modUserLoggedInDetails($pUserInfo);
					}
	...
	public function modDefaultCloset($pUid){
		try{
			...
			$userInfoArr = array();
			$userInfoArr['userId'] = 3;
			//Adore Product IDs - 12 products
			$userInfoArr['prodIds'] ="815|816|819|820|821|822|870|871|872|874|875|876";		
			//Lookbook Product IDs - 10 products
			$userInfoArr['prodIds'] .="|1423|1432|1433|1434|1435|1436|1437|1438|1439|1440";
			//Seemore Demo Product IDs - 22 products
			$userInfoArr['prodIds'] .= "|6065|6066|6067|6068|6069|6070|6071|6072|6073|6074|6075|6076|5889|5890|5891|5892|5894|6108|6109|6110|6111|6112";
	...
	public function modDefaultMyOffers($pUid){
		try{
			...
			$userInfoArr['userId'] = 3;
			//Adore Product IDs - 12 products
			$userInfoArr['offerIds'] ="47|15";		
			$this->modUserAddMyOffers($userInfoArr);
		}
		catch ( Exception $e ) {
			echo 'Message: ' .$e->getMessage();
		}
	...
	public function getMyOfferDetails($pOfferID){
		try{
			...
			for($i=0;$i<count($arrProdDetails);$i++){
				...
				$outArrMyOfferDetails[$i]["price"] = isset($arrMyOfferInfo[0]['offer_discount_value']) ? $arrMyOfferInfo[0]['offer_discount_value'] : 0;
...
// 			$pOfferID = 69;
			...
			for($i=0;$i<count($arrProdDetails);$i++){
				...
				// 	$outArrMyOfferDetails["price"] = 0.00;
				// }
				$outArrMyOfferDetails["price"] = isset($arrMyOfferInfo[$i]['offer_discount_value']) ? $arrMyOfferInfo[$i]['offer_discount_value'] : 0;
