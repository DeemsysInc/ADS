...
class mCloset extends SQLQuery {
	...
	{
		$query = "SELECT c.*,pd.*,cl.name AS client_name, cl.background_color, cl.light_color, cl.dark_color, cl.logo AS client_logo,cl.url AS client_url ,(SELECT group_concat(`relatedto_id` separator ',') as `related_id` FROM product_related AS pr WHERE pr.relatedfrom_id= pd.pd_id ) AS related_id FROM closet AS c 
		...
		$query .= "  ORDER BY c.closet_created_date DESC";
		//echo $query;
		$result = $this->selectQueryForAssoc($query);
		return $result;
	}
	public function getUserWithProductsWishListAndCloset($cValsArray)
	{
		 ...
		 */
		$query .= " GROUP BY pd_id ORDER BY pd_id ASC";
