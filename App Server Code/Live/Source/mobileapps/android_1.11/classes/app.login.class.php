...
class cAppLogin{
	...
	public function login_app_user($vArray){
		...
		if(count($checkLogin) >0){
			...
			//$arrResult['user_details_fbid'] = isset($checkLogin[0]['user_details_fbid']) ? $checkLogin[0]['user_details_fbid'] : 0;
			$con = array();
			$udArray = array();	
	...
	public function getInsertRegistrationDetails($pArray){
		try{
			...
			$pTableName = "users";
			$checkUserReg = $this->objLoginQuery->checkAppUserRegistered($pArray,$dArray);
			if(count($checkUserReg)==0){
				...
				if ($insertedUsersDetailsResultId){
