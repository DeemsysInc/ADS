...
class mCloset extends SQLQuery {
...

	{
		...
		$query = "SELECT c.*,pd.*,cl.name AS client_name, cl.background_color, cl.light_color, cl.dark_color, cl.logo AS client_logo FROM closet AS c LEFT JOIN products AS pd ON pd.pd_id = c.pd_id LEFT JOIN client AS cl ON pd.client_id = cl.id WHERE c.closet_status=1 AND c.user_id=".$cValsArray['userId']." AND pd.pd_id!=''";
