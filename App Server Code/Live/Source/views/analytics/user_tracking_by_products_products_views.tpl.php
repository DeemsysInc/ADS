	...
	<div class="block-border"><form class="block-content form" id="table_form" method="post" action="">
		...
		<table class="table sortable no-margin" cellspacing="0" width="100%" id="cms_clients">
			...
			<tbody>
				...
				<tr>
					<td><a href="<?php echo $config['LIVE_URL']; ?>analytics/mobile_users/product_views/clients/<?php echo isset($arrData[$i]['dates']) ? $arrData[$i]['dates'] : ''; ?>"><?php echo isset($arrData[$i]['dates']) ? $arrData[$i]['dates'] : ''; ?></a></td>
					<td><?php echo isset($arrData[$i]['productids']) ? $arrData[$i]['productids'] : ''; ?></td>
...
<script src="http://www.google.com/jsapi"></script>
  ...
  function drawChartProductsTrack() {
    ...
    data.addRows([
     ...
     {
         echo '["'.date('M j,Y',strtotime($arrData[$i]['dates'])).'", '.$arrData[$i]['productids'].'],';
     }
      ?>
    ]);
