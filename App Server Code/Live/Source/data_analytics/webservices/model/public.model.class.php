...
class mPublic extends SQLQuery {
	...
	public function getClientCampaigns($sendArray)
	{
		try 
		{
			...
			{
				$query = "SELECT * FROM  `client_campaigns` AS `cc`,`client` AS `c` WHERE   `cc`.`client_id` = `c`.`id`  AND `cc`.`client_id`=".$sendArray['login_client_id'];
		    				
		    
		    }
		    else if(($sendArray['group_id']==2) || ($sendArray['group_id']==1) || ($sendArray['group_id']==6))//super admin , admin , sales manager
		    {
		    	$query = "SELECT * FROM  `client_campaigns` AS `cc`,`client` AS `c` WHERE   `cc`.`client_id` = `c`.`id`  AND `cc`.`client_id`=".$sendArray['search_client_id'];
	...
	public function getClientAllOffersForDashboard($sendArray)
	{
		...
		{   
			if($sendArray['group_id']==3)//client
			{
				$query = "SELECT count(`moa`.`offer_id`) AS 'offerViews'  FROM `my_offers_analytics` as `moa`  WHERE `moa`.`datapoint_id`!=10 AND `datapoint_id`!=14 AND `moa`.`user_id`!=0 AND `moa`.`offer_id`!=0  AND `moa`.`client_id` IN(".$sendArray['search_client_id'].") AND  DATE_FORMAT(`moa`.`myoffers_created_date`, '%Y-%m-%d') BETWEEN '".$sendArray['start_date']."' AND '".$sendArray['end_date']."'";  
		    
			}
			else if(($sendArray['group_id']==2) || ($sendArray['group_id']==1) || ($sendArray['group_id']==6))
			{
				$query = "SELECT count(offer_id) AS 'offerViews'  FROM `my_offers_analytics` WHERE `datapoint_id`!=10 AND `datapoint_id`!=14 AND `user_id`!=0 AND `offer_id`!=0 AND `client_id` IN(".$sendArray['search_client_id'].") AND DATE_FORMAT(`myoffers_created_date`, '%Y-%m-%d') BETWEEN '".$sendArray['start_date']."' AND '".$sendArray['end_date']."'";  
	...
	public function getClientOfferUsersByDpId($sendArray)
	{
		try 
		{   
            $query = "SELECT user_id  FROM `my_offers_analytics` WHERE  `user_id`!=0 AND `datapoint_id`!=10  AND  DATE_FORMAT(`myoffers_created_date`, '%Y-%m-%d') BETWEEN '".$sendArray['start_date']."' AND '".$sendArray['end_date']."' AND  `offer_id`='".$sendArray['offer_id']."' GROUP BY `user_id`";  
		    $result = $this->selectQuery($query);
		    return $result;
		}
		catch ( Exception $e ) {
