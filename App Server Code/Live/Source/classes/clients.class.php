...
class cClients{
	...
	public function modAllClients(){
		try{
			global $config;
			$outArrAllClients = array();
			
			$outArrAllClients = $this->objClients->getAllClients();
			include SRV_ROOT.'views/clients/clients.tpl.php';
		}
		catch ( Exception $e ) {
			echo 'Message: ' .$e->getMessage();
		}
	...
	public function modAjaxSaveClientProduct(){
		try{
				...
				if(isset($outArrGetClientProducts[0]['pd_image']) && $outArrGetClientProducts[0]['pd_image']!='')
				{
					...
					if ( !file_exists($dstfile) ) {
					  ...
					  chmod($dstfile, 0777);
						...
						chmod($srcfile, 0777);				
					//copy
					if (copy($srcfile,$dstfile.$outArrGetClientProducts[0]['ipd_mage'])) {
			...
			$uArray=array();
			$uArray['pd_bg_color']=isset($_REQUEST['hdn_bcid']) ? $_REQUEST['hdn_bcid'] : '';
	...
	public function modClientEditDetails($cid){
		try{
			global $config;
			...
			include SRV_ROOT.'views/clients/client_edit.tpl.php';
		}
		catch ( Exception $e ) {
			echo 'Message: ' .$e->getMessage();
		}
	...
	public function modShowClientForm(){
		try{
			global $config;
			$outArrAllVerticalClients = array();
			$outArrAllVerticalClients =$this->objClients->getVerticalClients();
			include SRV_ROOT.'views/clients/client_add_form.tpl.php';
		}
		catch ( Exception $e ) {
			echo 'Message: ' .$e->getMessage();
		}
...
//								
	...
	public function modAjaxSaveButtonVisual(){
		try{
			...
			$pArray['product_id']=isset($_REQUEST['pid']) ? $_REQUEST['pid'] : '';
			...
			else
			{
				...
				$arrData['product_id']=isset($_REQUEST['pid']) ? $_REQUEST['pid'] : 0;
				$con=array();
				$con['id']=$visual_id;
	...
	public function modAjaxSave3DModelVisual(){
		try{
			...
			$pArray['animate_on_recognition']=isset($_REQUEST['animate_recog']) ? $_REQUEST['animate_recog'] : '';
			...
			else
			{
			    ...
			    $arrData['animate_on_recognition']=isset($_REQUEST['animate_recog']) ? $_REQUEST['animate_recog'] : '';
