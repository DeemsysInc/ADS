...
class mProductWishlist extends SQLQuery {
	...
	{
		$query = "SELECT wl.wishlist_id, wl.wishlist_name, wl.user_id, wld.wishlist_details_id, wld.pd_id AS prodId, pd.client_id, pd.pd_name, 
		pd.pd_image FROM `wishlist` AS wl 
		...
		if($wishListName!=""){
			...
			$query .= " AND wl.wishlist_name=(SELECT wishlist_name FROM wishlist WHERE user_id=164 ORDER BY wishlist_id ASC LIMIT 0, 1) ";
		}		
	...
	{
		$query = "SELECT wl.name AS wishListName, pd.client_id, pd.title, pd.image, pd.id AS prodId, wli.wish_list_id FROM  `wish_list` AS wl LEFT JOIN wish_list_item AS wli ON wli.wish_list_id=wl.id LEFT JOIN product AS pd ON pd.id=wli.product_id AND pd.active=1 WHERE wli.product_id!='' AND wl.user_id=".$userId." GROUP BY wli.product_id LIMIT 0, 10";
		$result = $this->selectQueryForAssoc($query);
		return $result;
	}
	...
	{
		 $query = "SELECT cl.name AS  `client_name` , wl.id,wl.shared,wli.id AS `item_id`, wl.name AS wishListName, pd.client_id,pd.price, pd.title, pd.image, pd.id AS prodId FROM  `wish_list` AS wl LEFT JOIN wish_list_item AS wli ON wli.wish_list_id=wl.id LEFT JOIN product AS pd ON pd.id=wli.product_id AND pd.active=1 LEFT JOIN  `client` AS cl ON cl.id = pd.client_id WHERE wl.user_id !='".$userId."'  AND wl.id='".$wishListId."' AND  wl.shared = 1 GROUP BY wli.product_id ORDER BY wli.product_id DESC";
