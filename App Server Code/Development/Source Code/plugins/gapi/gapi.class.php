...
class gapi
{
  ...
  public function requestReportData($report_id, $dimensions, $metrics, $sort_metric=null, $filter=null, $start_date=null, $end_date=null, $start_index=1, $max_results=30)
  {
    ...
    if($filter!=null)
    {
		...
		{
		  $filter_string = '';
		  foreach($filter as $arrFilter)
		  {
			//$filter_string .= ',' . $this->processFilter($arrFilter);
			$filter_string .= ',' . $arrFilter;
		  }
		  $parameters['filters'] = substr($filter_string,1);
		}
		else 
		{
		 $parameters['filters'] =$arrFilter;
		}
