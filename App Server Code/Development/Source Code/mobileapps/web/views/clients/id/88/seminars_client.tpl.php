...
formatDate:'m/d/Y'
	...
	function showPosition(position) {
		var currentlatlng= position.coords.latitude + 
		"," + position.coords.longitude;
		$('#latlng').val(currentlatlng);
		initialize({'lat':position.coords.latitude,'lng':position.coords.longitude})
	}
	var map;
	var infowindow;
	var currentLatLng=$("#latlng").val();

	var arraylatlng=currentLatLng.split(",");
	var currentlat=arraylatlng[0];
	var currentlon=arraylatlng[1];

	var lat = parseFloat(currentlat);  //convert string to float
	var lon = parseFloat(currentlon);  //convert string to float

	function initialize(coords) {
		
	
		
	  var pyrmont = new google.maps.LatLng(coords.lat,coords.lng);
	...
	function createMarker(place) {
	  ...
	  google.maps.event.addListener(markerA, 'click', function() {
		infowindow.setContent("sdfsdfksdf");
	...
	//google.maps.event.addDomListener(window, 'load', initialize({'lat':lat,'lng':lon}));
    </script>
