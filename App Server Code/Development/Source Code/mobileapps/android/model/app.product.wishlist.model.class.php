...
class mProductWishlist extends SQLQuery {
	...
	public function getUserWishListProducts($userId,$wishListName)
	{
		...
		IFNULL(pd.pd_istryon, '0' ) AS pd_istryon,cl.background_color,cl.name AS client_name, cl.light_color, cl.dark_color, cl.logo AS client_logo,cl.url AS client_url FROM `wishlist` AS wl LEFT JOIN wishlist_details AS wld ON wld.wishlist_id=wl.wishlist_id AND wld.wishlist_details_status=1 LEFT JOIN products AS pd ON pd.pd_id=wld.pd_id AND pd.pd_status=1 LEFT JOIN client AS cl ON pd.client_id = cl.id 
