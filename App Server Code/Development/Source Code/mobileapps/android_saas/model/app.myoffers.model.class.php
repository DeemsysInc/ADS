...
class mMyOffers extends SQLQuery {
	...
	{
		global $config;
		$query = "SELECT * FROM ".$this->usersDbName.".`my_offers` WHERE my_offers_name='".$mArray['my_offers_name']."' AND user_id=".$mArray['user_id']." LIMIT 0, 1";
	...
	{
		global $config;
		$query = "SELECT * FROM ".$this->usersDbName.".`my_offers` WHERE user_id=".$loggedInUserId." LIMIT 0, 1";
		$result = $this->selectQueryForAssoc($query);
		return $result;
	}
	...
	{
		global $config;
		$query = "SELECT * FROM ".$this->usersDbName.".`my_offers` AS m LEFT JOIN ".$this->usersDbName.".`my_offers_reference` AS mor ON mor.my_offers_id=m.my_offers_id WHERE user_id=".$loggedInUserId." LIMIT 0, 1";
		$result = $this->selectQueryForAssoc($query);
		return $result;
	}*/
	...
	{
		global $config;
		$query = "SELECT * FROM ".$this->usersDbName.".`my_offers_reference` WHERE my_offers_id='".$mArray['my_offers_id']."' AND offer_id=".$mArray['offer_id']." LIMIT 0, 1";
		$result = $this->selectQueryForAssoc($query);
		return $result;
	}
	function getClientWithOffersAndInfoDetails($clientId, $loggedInUserId, $offerId)
	{
		...
		FROM offers AS of 
		LEFT JOIN ".$config['database']['name_clients'].".`client` AS cl ON cl.id=of.client_id
		...
		LEFT JOIN ".$config['database']['name_clients'].".`offers_info` AS oi ON oi.offer_id=of.offer_id
	...
	function getClientWithOfferDetailsWithUserIdOld($loggedInUserId, $offerId, $flag)
	{
		...
		FROM ".$this->usersDbName.".`my_offers` AS mof 
		LEFT JOIN ".$this->usersDbName.".`my_offers_reference` AS mofr ON mofr.my_offers_id=mof.my_offers_id AND mofr.my_offers_ref_status=1 ";		
		if($offerId!=""){
			$query .= " AND mofr.offer_id=".$offerId." "; 
		}
		$query .= " LEFT JOIN offers AS of ON of.offer_id=mofr.offer_id AND of.offer_status=1 
		LEFT JOIN ".$config['database']['name_clients'].".`offers_info` AS oi ON oi.offer_id=of.offer_id AND oi.offer_info_status=1 
	...
	function getClientWithOfferDetailsWithUserIdOrOfferID($loggedInUserId, $offerId, $flag)
	{
		...
		$query = "SELECT cl.id, cl.name,cl.client_vertical_id, mof.my_offers_name,mof.my_offers_created_date, mof.user_id, mofr.my_offers_ref_id, of.offer_id, of.client_id, cl.background_color, cl.light_color, cl.dark_color, 
		...
		$query .= " LEFT JOIN ".$config['database']['name_clients'].".`offers` AS of ON of.offer_id=mofr.offer_id AND of.offer_status=1 ";
		if($flag=="expiration"){
			$query .= " AND of.offer_valid_to > NOW()";
		}
		$query .= " LEFT JOIN ".$config['database']['name_clients'].".`offers_info` AS oi ON oi.offer_id=of.offer_id AND oi.offer_info_status=1
		LEFT JOIN ".$config['database']['name_clients'].".`offers_discount_type` AS ofdt ON ofdt.offer_discount_type_id=of.offer_discount_type_id AND ofdt.offer_discount_status=1 
		LEFT JOIN ".$config['database']['name_clients'].".`client` AS cl ON cl.id=of.client_id AND cl.active=1
		LEFT JOIN ".$config['database']['name_clients'].".`client_details` as `cld` ON `cl`.`id` = `cld`.`client_id` AND `cld`.client_details_status=1 
		LEFT JOIN ".$this->mainDbName.".`countries` AS `cs` ON `cld`.`client_details_country_code` = `cs`.`country_code_char2` AND `cld`.`client_details_currency_code` = `cs`.`currency_code`
	...
	function getClientWithRelatedOffers($clientId, $offerId, $layoutType)
	{
		...
		$query .= "
		FROM ".$config['database']['name_clients'].".`offers_related` AS `ofr`
		...
		LEFT JOIN `offers_discount_type` AS odt ON odt.client_id=of.client_id AND odt.offer_discount_status =1 	AND odt.offer_discount_type_id =of. offer_discount_type_id	
		LEFT JOIN ".$config['database']['name_clients'].".`products` AS pd ON pd.pd_id=`ofr`.relatedto_prodid AND pd.client_id=".$clientId." AND pd.pd_status=1 
		LEFT JOIN ".$config['database']['name_clients'].".`client` AS cl ON (cl.id=of.client_id OR cl.id=pd.client_id ) AND cl.active=1  
		LEFT JOIN ".$config['database']['name_clients'].".`client_details` as `cld` ON `cl`.`id` = `cld`.`client_id` AND `cld`.client_details_status=1  AND `cld`.`client_details_currency_code` = `cs`.`currency_code`
