...
class mPublic extends SQLQuery {
	...
	function getAllTriggers_new()
	{
		...
		$query = "SELECT tr.id AS trigger_id, tr.url AS triggerUrl, tr.client_id, tr.title AS triggerTitle, tr.height AS triggerHeight, tr.trigger_by_vertical,  
			...
			FROM ".$this->markersDbName.".`trigger` as `tr` 
			LEFT JOIN ".$this->markersDbName.".`visual` as `vi` ON `vi`.`trigger_id` = `tr`.id AND `vi`.`discriminator` !=''
			LEFT JOIN ".$config['database']['name_clients'].".`products` as `pd` ON `vi`.`product_id` = `pd`.`pd_id` AND `pd`.`pd_status`=1 
			LEFT JOIN ".$config['database']['name_clients'].".`client` as `cl` ON `tr`.`client_id` = `cl`.`id` AND `cl`.active=1
			LEFT JOIN ".$config['database']['name_clients'].".`client_details` as `cld` ON `cl`.`id` = `cld`.`client_id` AND `cld`.client_details_status=1 
			LEFT JOIN ".$this->mainDbName.".`countries` AS `cs` ON `cld`.`client_details_country_code` = `cs`.`country_code_char2` AND `cld`.`client_details_currency_code` = `cs`.`currency_code`
			LEFT JOIN ".$config['database']['name_clients'].".`products_addtional_image` AS `pai` ON pai.pd_id=pd.pd_id AND pai.pd_additional_status=1 
			LEFT JOIN ".$config['database']['name_clients'].".`offers` AS of ON vi.offer_id=of.offer_id AND of.offer_status=1 
			LEFT JOIN ".$config['database']['name_clients'].".`offers_info` AS oi ON oi.client_id=of.client_id AND oi.offer_id=of.offer_id 
	...
	function getClientProducts($clientId, $productId)
	{
		...
		FROM ".$config['database']['name_clients'].".`products` AS pd 
		LEFT JOIN ".$config['database']['name_clients'].".`client` AS cl ON cl.id=pd.client_id AND cl.active=1 
		LEFT JOIN ".$config['database']['name_clients'].".`client_details` as `cld` ON `cl`.`id` = `cld`.`client_id` AND `cld`.client_details_status=1 
		LEFT JOIN ".$this->mainDbName.".`countries` AS `cs` ON `cld`.`client_details_country_code` = `cs`.`country_code_char2` AND `cld`.`client_details_currency_code` = `cs`.`currency_code`
		LEFT JOIN ".$config['database']['name_clients'].".`product_related` AS pdr ON pd.pd_id=pdr.relatedfrom_id 
		LEFT JOIN ".$config['database']['name_clients'].".`products_addtional_image` AS pai ON pai.pd_id=pd.pd_id AND pai.pd_additional_status=1 
	...
	function getClientWithRelatedProducts($clientId, $productId, $layoutType)
	{
		...
		$query .= "FROM ".$config['database']['name_clients'].".`product_related` AS pdr 
		LEFT JOIN ".$config['database']['name_clients'].".`products` AS pd ON pd.pd_id=pdr.relatedto_id AND pd.client_id=".$clientId." AND pd.pd_status=1 
		LEFT JOIN ".$config['database']['name_clients'].".`products_addtional_image` AS pai ON pai.pd_id=pd.pd_id AND pai.pd_additional_status=1 
		LEFT JOIN ".$config['database']['name_clients'].".`offers` AS of ON of.offer_id=pdr.relatedto_offerid AND of.offer_status=1
		LEFT JOIN ".$config['database']['name_clients'].".`offers_discount_type` AS odt ON odt.client_id=of.client_id AND odt.offer_discount_status =1
		LEFT JOIN ".$config['database']['name_clients'].".`client` AS cl ON (cl.id=of.client_id OR cl.id=pd.client_id ) AND cl.active=1 
		LEFT JOIN ".$config['database']['name_clients'].".`client_details` as `cld` ON `cl`.`id` = `cld`.`client_id` AND `cld`.client_details_status=1
		LEFT JOIN ".$this->mainDbName.".`countries` AS `cs` ON `cld`.`client_details_country_code` = `cs`.`country_code_char2` ";
...
*/
	...
	{
		global $config;		
		$query = "SELECT *, (SELECT (COUNT(model)+COUNT(texture)+COUNT(material)) FROM ".$this->markersDbName.".model WHERE three_d_model_id=".$triggerId.") AS totalModelCount FROM ".$this->markersDbName.".`model` WHERE three_d_model_id=".$triggerId."";
		$result = $this->selectQueryForAssoc($query);
		return $result;
	}
	...
	function getTapForDetails($loggedInUserId, $productId)
	{
		...
		FROM ".$config['database']['name_clients'].".`products_addtional_image` AS pai 
		LEFT JOIN ".$config['database']['name_clients'].".`products` AS pd ON pd.pd_id=pai.pd_id AND pd.pd_status=1 
	...
	function getClientWithProductDetails($clientId, $productId)
	{
		global $config;
		$query = "SELECT pd.pd_id AS prodId, pd.pd_image AS pdImage, pd.`pd_price` AS pdPrice, pd.`pd_name` AS `prodName`, 
		pd.client_id, cl.id AS clientId, cl.name AS clientName, cl.background_color, cl.light_color, cl.dark_color, cld.client_details_country_code, cs.country_name, CASE WHEN cs.country_code_char2 IS NULL or cs.country_code_char2 = '' THEN 'US' ELSE cs.country_code_char2 END AS country_code_char2, CASE WHEN cs.country_languages IS NULL or cs.country_languages = '' THEN 'en' ELSE cs.country_languages END AS country_languages, cs.currency_code, 
		IFNULL(pd.pd_short_description, 'null' ) AS pd_short_description, IFNULL(cl.url, 'null' ) AS clientUrl, 
	...
	{
		global $config;
		$query = "SELECT *, image AS clientImage FROM ".$this->mainDbName.".`client_groups` WHERE active=1 ORDER BY id ASC";		
		$result = $this->selectQueryForAssoc($query);
		return $result;
	}
	function getClientAllTriggersForXML($clientId)
	{
		...
		$query = "SELECT tr.id AS trigger_id, tr.url AS triggerUrl, tr.client_id, tr.title AS triggerTitle, tr.height AS triggerHeight, tr.trigger_by_vertical, 
			...
			LEFT JOIN ".$config['database']['name_clients'].".`products_background_view` AS `pwv` ON pwv.pd_id=vi.product_id AND pwv.pd_bg_status=1 
			LEFT JOIN ".$config['database']['name_clients'].".`offers` AS of ON vi.offer_id=of.offer_id AND of.offer_status=1 
			LEFT JOIN ".$config['database']['name_clients'].".`offers_info` AS oi ON oi.client_id=of.client_id AND oi.offer_id=of.offer_id 
	...
	{
		global $config;
		$query = "SELECT * FROM ".$this->usersDbName.".`users` AS u, ".$this->usersDbName.".`user_details` AS ud WHERE u.user_id = ud.user_id AND u.user_id =".$userId;
		//echo $query;
		$result = $this->selectQueryForAssoc($query);
		return $result;
	}
	...
	{
		global $config;
		$query = "SELECT * FROM ".$config['database']['name_clients'].".`client_stores`  AS cs LEFT JOIN ".$config['database']['name_clients'].".`client` AS cl ON cl.id=cs.client_id AND cl.active=1 WHERE client_id IN (".$clientIds.")";
		$result = $this->selectQueryForAssoc($query);
		return $result;
	}
	function getClientWithTryonRelatedProducts($clientId, $productId)
	{
		...
		FROM ".$config['database']['name_clients'].".`product_related` AS pdr 
		LEFT JOIN ".$config['database']['name_clients'].".`products` AS pd ON pd.pd_id=pdr.relatedto_id AND pd.client_id=".$clientId." AND pd.pd_status=1 
		LEFT JOIN ".$config['database']['name_clients'].".`client` AS cl ON cl.id=pd.client_id AND cl.active=1 		
		LEFT JOIN ".$config['database']['name_clients'].".`client_details` as `cld` ON `cl`.`id` = `cld`.`client_id` AND `cld`.client_details_status=1 
		LEFT JOIN ".$this->mainDbName.".`countries` AS `cs` ON `cld`.`client_details_country_code` = `cs`.`country_code_char2` AND `cld`.`client_details_currency_code` = `cs`.`currency_code`
