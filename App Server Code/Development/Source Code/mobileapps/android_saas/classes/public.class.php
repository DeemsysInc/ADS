...
class cPublic{
	...
	public function modClientGroupTriggersWithXML(){
		try{
			...
			if(count($outAllTriggers['trigger'])){
	...
	public function modClientTriggersWithXML($pUrl, $clientIds){
	 ...
	 public function modUserDetails($userId){
		try{
			...
			//echo json_encode($outResults);
			//echo count($outResults['resultXml']);
			if(count($outResults['resultXml'])>0){
			...
			$dArray['user_details_lastlogin'] = 'NOW()';
				...
				if (count($arrUser) > 0){
					if (count($pArray)>0){
						...
						$con['user_status'] = 1;	
						if($con['user_id'] !=0){
							$updateRecord = $this->objPublic->updateRecordQuery($pArray, $this->usersDbName.".`users`", $con);
							if ($updateRecord){
					...
					if (count($dArray)>0){
							...
							$con['user_id'] = $userId;		
							if($con['user_id'] !=0){
								$updateRecord = $this->objPublic->updateRecordQuery($dArray, $this->usersDbName.".`users`", $con);
								if ($updateRecord){
