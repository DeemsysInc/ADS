...
class cAppLogin{
	...
	public function getAccessWithAppLoginDetails($pArray){
		try{
			...
			$vArray['user_details_fbid'] = isset($_REQUEST['user_details_fbid']) ? $_REQUEST['user_details_fbid'] : (isset($pArray[7]) ? $pArray[7] : '');
			$vArray['user_firstname'] = isset($_REQUEST['first_name']) ? $_REQUEST['first_name'] : (isset($pArray[8]) ? $pArray[8] : '');
			$vArray['user_lastname'] = isset($_REQUEST['last_name']) ? $_REQUEST['last_name'] : (isset($pArray[9]) ? $pArray[9] : '');
	...
	public function login_app_user($vArray){
		...
		if($vArray['checkUrlFlag']=="login_profiling"){
			...
			if($vArray['uname'] =="" && $vArray['email_id'] ==""){
				$checkLogin = $this->objLoginQuery->checkAppUserRegisteredByName($vArray);				
			}else{
	...
	public function getInsertRegistrationDetails($pArray){
		try{
			...
			}else if($pArray['user_firstname'] !="" && $pArray['user_lastname'] !="" && $pArray['user_details_fbid'] !=""){
				$pArray['user_details_fbid'] = isset($_REQUEST['user_details_fbid']) ? $_REQUEST['user_details_fbid'] : '';
				$checkUserReg = $this->objLoginQuery->checkAppUserRegisteredByName($pArray);
			}
