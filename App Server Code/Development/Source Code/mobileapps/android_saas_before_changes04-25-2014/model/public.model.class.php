...
*/
	...
	{
		global $config;
		$this->clientDbName = $config['database']['name_clients'];
		$query = "SELECT tr.id AS trigger_id, tr.url AS triggerUrl, tr.client_id, tr.title AS triggerTitle, tr.height AS triggerHeight, tr.trigger_by_vertical, 
			tr.width AS triggerWidth, `vi`.`url` AS visualUrl, `pd`.`pd_image` AS pdImage,
			...
			LEFT JOIN ".$this->clientDbName.".`products` as `pd` ON `vi`.`product_id` = `pd`.`pd_id` AND `pd`.`pd_status`=1 
			LEFT JOIN ".$this->clientDbName.".`client` as `cl` ON `tr`.`client_id` = `cl`.`id` AND `cl`.active=1
			
			LEFT JOIN ".$this->clientDbName.".`client_details` as `cld` ON `cl`.`id` = `cld`.`client_id` AND `cld`.client_details_status=1
			...
			LEFT JOIN ".$this->clientDbName.".`products_addtional_image` AS `pai` ON pai.pd_id=pd.pd_id AND pai.pd_additional_status=1 
			
			LEFT JOIN ".$this->clientDbName.".products_background_view AS `pwv` ON pwv.pd_id=vi.product_id AND pwv.pd_bg_status=1 
			LEFT JOIN ".$this->clientDbName.".`offers` AS of ON vi.offer_id=of.offer_id AND of.offer_status=1 
			LEFT JOIN ".$this->clientDbName.".`offers_info` AS oi ON oi.client_id=of.client_id AND oi.offer_id=of.offer_id 
			
			LEFT JOIN ".$this->clientDbName.".`offers_discount_type` AS odt ON odt.client_id=of.client_id AND odt.offer_discount_status =1 
