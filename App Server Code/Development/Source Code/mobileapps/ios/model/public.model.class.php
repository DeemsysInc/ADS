...
// 	}
	...
	// 	$query = "SELECT * FROM `trigger` WHERE ";
	...
	// }
	function getClientTriggers($arrClient){
		$query = "SELECT *,`t`.`id` AS `triggerId`, `t`.`url` AS `triggerUrl` FROM `trigger` AS `t`, `client` AS `c` WHERE `t`.`client_id` = `c`.`id` AND ";
		if (count($arrClient)>0){
			$query .= " `client_id` IN (";
			for ($i=0;$i<count($arrClient);$i++){
				$query .= $arrClient[$i];
	...
	function getTriggerByTid($triggerId){
		$query = "SELECT *,`t`.`id` AS `triggerId`, `t`.`url` AS `triggerUrl` FROM `trigger` AS `t`, `client` AS `c` WHERE `t`.`client_id` = `c`.`id` AND  
		`t`.`id` = $triggerId AND 
		`t`.`active` = 1";
	...
	function getClientGroupTriggerIDs(){
		$query = "SELECT (`id`+90000) AS `id` ,`client_id`, `trigger_id` FROM `client_groups` WHERE `active` = 1";
		$result = $this->selectQueryForAssoc($query);
		return $result;
	...
	function getAdditionalImagesByOfferId($offerId){
		$query = "SELECT offer_additional_media FROM `offers_additional` WHERE `offer_id` IN ($offerId) AND `offer_additional_status` = 1";
		$result = $this->selectQueryForAssoc($query);
		return $result;	
	}
	...
	// function getUserDetailsByUname($pUserInfo){
	...
	// 			`u`.`user_id` = `ud`.`user_id` 
	// 		AND `u`.`user_group_id` = `ug`.`user_group_id` 
	// 		AND `u`.`user_username` = '$uname' 
	// 		AND `u`.`user_status` = 1";
	// 		if ($fbid !='') {
	// 			$query .= " AND `u`.`user_register_through` = 1";
	// 		}
	// 		echo $query;
	// 	$result = $this->selectQueryForAssoc($query);
	// 	return $result;	
	...
	function getUserDetailsByUname($pUname){
		$query = "SELECT * FROM `users` AS `u`, `user_details` AS `ud`, `user_group` AS `ug` WHERE  
			...
			AND `u`.`user_status` = 1";
			// echo 'getUserDetailsByUname '.$query;
		$result = $this->selectQueryForAssoc($query);
		return $result;	
	}
	function getUserDetailsByEmail($pEmail){
		$query = "SELECT * FROM `users` AS `u`, `user_details` AS `ud`, `user_group` AS `ug` WHERE  
				`u`.`user_id` = `ud`.`user_id` 
			AND `u`.`user_group_id` = `ug`.`user_group_id` 
			AND `u`.`user_email_id` = '$pEmail' 
			AND `u`.`user_status` = 1";
			// echo 'getUserDetailsByUname '.$query;
		$result = $this->selectQueryForAssoc($query);
		return $result;	
	...
	function getOfferInfoByOfferId($pOfferID){
		 $query = "SELECT * FROM `offers_info` AS `oi`, `client` AS `c`, `offers_discount_type` AS `odt`, `offers` AS `o` WHERE 
		`oi`.`client_id` = `c`.`id` AND `oi`.`offer_id` = `o`.`offer_id` AND `o`.`offer_discount_type_id` = `odt`.`offer_discount_type_id` AND
		`o`.`offer_id` = $pOfferID";
		$result = $this->selectQueryForAssoc($query);
		return $result;	
	}
	...
	function checkIfClosetExistsByUidDefault($pUid,$pdId){
		$query = "SELECT * FROM `closet_testing` WHERE `user_id` = $pUid AND `pd_id`=$pdId";
		$result = $this->selectQueryForAssoc($query);
		return $result;
	}
	...
	function checkIfUsernameExistsByEmail($pUserEmail){
		$query = "SELECT * FROM `users` WHERE `user_email_id` = '$pUserEmail'";
		$result = $this->selectQueryForAssoc($query);
		return $result;
	}

	function checkIfUsernameExistsByUnameUsingFB($pUserInfo){
		$uname = isset($pUserInfo['uname']) ? $pUserInfo['uname'] : '';
		$fbid = isset($pUserInfo['fbid']) ? $pUserInfo['fbid'] : '';
		$email = isset($pUserInfo['email']) ? $pUserInfo['email'] : '';
		$query = "SELECT * FROM `users` AS `u`, `user_details` AS `ud` WHERE `u`.`user_id` = `ud`.`user_id` AND `u`.`user_email_id` = '$email' AND `u`.`user_register_through` = 1 AND `ud`.`user_details_fbid` = $fbid";
		$result = $this->selectQueryForAssoc($query);
		return $result;
	}
	
	function checkIfUsernameExistsByUnameUsingType($pUserInfo){
		$uname = isset($pUserInfo['uname']) ? $pUserInfo['uname'] : '';
		$fbid = isset($pUserInfo['fbid']) ? $pUserInfo['fbid'] : '';
		$email = isset($pUserInfo['email']) ? $pUserInfo['email'] : '';
		$query = "SELECT * FROM `users` AS `u`, `user_details` AS `ud` WHERE `u`.`user_id` = `ud`.`user_id` AND `u`.`user_email_id` = '$email' AND `u`.`user_register_through` = 1";
		$result = $this->selectQueryForAssoc($query);
		return $result;
	}
	function checkIfUsernameExistsByFBID($pUserInfo){
		$uname = isset($pUserInfo['uname']) ? $pUserInfo['uname'] : '';
		$fbid = isset($pUserInfo['fbid']) ? $pUserInfo['fbid'] : '';
		$email = isset($pUserInfo['email']) ? $pUserInfo['email'] : '';
		$query = "SELECT * FROM `users` AS `u`, `user_details` AS `ud` WHERE `u`.`user_id` = `ud`.`user_id` AND `ud`.`user_details_fbid` = '$fbid' AND `u`.`user_register_through` = 1";
		$result = $this->selectQueryForAssoc($query);
		return $result;
	}
	function checkIfUsernameExistsByFBIDAndNames($pUserInfo){
		$fname = isset($pUserInfo['fname']) ? $pUserInfo['fname'] : '';
		$fbid = isset($pUserInfo['fbid']) ? $pUserInfo['fbid'] : '';
		$lname = isset($pUserInfo['lname']) ? $pUserInfo['lname'] : '';
		$query = "SELECT * FROM `users` AS `u`, `user_details` AS `ud` WHERE `u`.`user_id` = `ud`.`user_id` AND `ud`.`user_details_fbid` = '$fbid' AND `u`.`user_firstname` = '$fname' AND `u`.`user_lastname` = '$lname' AND `u`.`user_register_through` = 1";
		$result = $this->selectQueryForAssoc($query);
		return $result;
	}
	function getRelatedOffersById($offerId){
		$query = "SELECT GROUP_CONCAT(DISTINCT relatedto_id ORDER BY relatedto_id ASC SEPARATOR ',') AS offerRelated, GROUP_CONCAT(DISTINCT relatedto_prodid ORDER BY relatedto_prodid ASC SEPARATOR ',') AS prodRelated FROM offers_related WHERE `relatedfrom_id`=$offerId";
		$result = $this->selectQueryForAssoc($query);
		return $result;	
	}
	function getClientStores($pClientId){
		$query = "SELECT * FROM `client_stores` AS `cs`, `client` AS `c`  WHERE `c`.`id` =  `cs`.`client_id` AND `cs`.`client_id` = '$pClientId'";
		$result = $this->selectQueryForAssoc($query);
		return $result;
	}
	function getCurrencyCodeByClientId($pClientId){
		$query = "SELECT * FROM `client_details` WHERE `client_id` = '$pClientId'";
		$result = $this->selectQueryForAssoc($query);
		return $result;
	}
	function getChangeLog($pUpdateDate){
		$query = "SELECT * FROM `change_log` WHERE `created_date` >= '$pUpdateDate'";
		$result = $this->selectQueryForAssoc($query);
		return $result;
	}
