...
class cPublic{
		...
		try{
			...
			if($productId != ""){
				...
				for($i=0;$i<count($outArrClientProduct);$i++)
				{
				 ...
				 $outArrClientProduct[$i]["offer_back_image"] = "http://".$_SERVER['HTTP_HOST']."/files/clients/".$outArrClientProduct[$i]["client_id"]."/products/".$outArrClientProduct[$i]["offer_back_image"];
				}
	...
	public function modTriggerByTId($triggerId){
		try{
			...
			$arrAllTriggers = $this->objPublic->getTriggerByTid($triggerId);
			
			// print_r($arrAllTriggers);
			
			for($i=0;$i<count($arrAllTriggers);$i++){
				// $arrClientDetailsById = $this->objPublic->getClientById($clientID);
				$clientName = isset($arrAllTriggers[$i]['name']) ? $arrAllTriggers[$i]['name'] : '';
				$triggerId = isset($arrAllTriggers[$i]['triggerId']) ? $arrAllTriggers[$i]['triggerId'] : 0;
				$outArrAllTriggers[$i]['triggerId'] = $triggerId;
				$triggerUrl = isset($arrAllTriggers[$i]['triggerUrl']) ? $arrAllTriggers[$i]['triggerUrl'] : '';
				$outArrAllTriggers[$i]['triggerURL'] = '';
				if ($triggerUrl != '') {
					$outArrAllTriggers[$i]['triggerURL'] = str_replace("{client_id}",$arrAllTriggers[$i]['client_id'],$config['files']['triggers']).$triggerUrl;
				}
				...
				$outArrAllTriggers[$i]['clientName'] = $clientName;
				...
				$arrVisualsByTriggerID = $this->objPublic->getVisualsByTriggerID($triggerId);
				for($j=0;$j<count($arrVisualsByTriggerID);$j++){
					...
					$outArrAllTriggers[$i]['visual'][$j]['client_game_id'] = isset($arrVisualsByTriggerID[$j]['client_game_id']) ? $arrVisualsByTriggerID[$j]['client_game_id'] : 0;
					
					//games info
					if($arrVisualsByTriggerID[$j]['discriminator'] == "GAMES"){
						/* $arrGamesByClientGameID = $this->objGames->modClientGameDetailsByCGID($arrVisualsByTriggerID[$j]['client_game_id']);
						$outArrAllTriggers[$i]['visual'][$j]['client_game_details']= $arrGamesByClientGameID;
						 */
						$sendArray=array();
						$sendArray['client_game_id'] = $arrVisualsByTriggerID[$j]['client_game_id'];
						...
						$arrData['game_id']=isset($outArrGameDetails[0]['game_id']) ? $outArrGameDetails[0]['game_id'] : 0;
						$arrData['client_id']=isset($outArrGameDetails[0]['client_id']) ? $outArrGameDetails[0]['client_id'] : 0;
						...
						$arrData['image']="http://".$_SERVER['HTTP_HOST']."/files/clients/".$arrData['client_id']."/games/".$outArrGameDetails[0]['game_id']."/".$instructionsImage;
						
						$outArrAllTriggers[$i]['visual'][$j]['client_game_details']= $arrData;
					}
			...
			// print_r($outArrAllTriggers);
			// echo 'count of triggers: '.count($outArrAllTriggers);
			if($fileType=="json")
			{
...
// 	}
		...
		try{
			...
			for($i=0;$i<count($arrAllClient);$i++){
				...
				for($j=0;$j<count($arrAllClientProducts);$j++){
					...
					$outArrAllClient['products'][$j]["isSeeMoreCart"] = isset($arrAllClientProducts[$j]['pd_issmcart']) ? $arrAllClientProducts[$j]['pd_issmcart'] : 0;
	...
	public function getProductDetails($pId){
		try{
			...
			$outArrProdDetails["isSeeMoreCart"] = isset($arrProdDetails[0]['pd_issmcart']) ? $arrProdDetails[0]['pd_issmcart'] : 1;
				...
				if (count($arrUser) > 0){
				 ...
				 if($outArrUser['msg']=='success'){
					if (count($arrUser) > 0){
						...
						$uname = isset($pUserInfo['uname']) ? $pUserInfo['uname'] : '';
						if ($uid > 0) {
							$outArrUser = $this->modUserLoggedInDetailsByUID($uid);
						}else{
							$outArrUser = $this->modUserLoggedInDetails($pUserInfo);
						}
	...
	public function modUserMyOffers($pUserInfo){
		try{
			//print_r($pUserInfo);
			 ...
			 $uid = isset($pUserInfo['userid']) ? $pUserInfo['userid'] : 9;
	...
	public function getMyOfferDetails($pOfferID){
		try{
			...
			for($i=0;$i<count($arrProdDetails);$i++){
				...
				$outArrMyOfferDetails[$i]["offerBackImage"] = '';
				$prodBackImage = isset($arrProdDetails[$i]['offer_back_image']) ? $arrProdDetails[$i]['offer_back_image'] : "";
				if ($prodBackImage !='') {
					$outArrMyOfferDetails[$i]["offerBackImage"] = str_replace("{client_id}",$arrProdDetails[$i]['client_id'],$config['files']['products']).$prodBackImage;
				}
				...
				$outArrMyOfferDetails[$i]["isOfferMultiRedeem"] = isset($arrProdDetails[$i]['offer_is_multi_redeem']) ? $arrProdDetails[$i]['offer_is_multi_redeem'] : 0;
...
// 			$pOfferID = 69;
			...
			for($i=0;$i<count($arrProdDetails);$i++){
				...
				$outArrMyOfferDetails["image"] = '';
				$prodImage = isset($arrProdDetails[$i]['offer_image']) ? $arrProdDetails[$i]['offer_image'] : "";
				if ($prodImage !='') {
					$outArrMyOfferDetails["image"] = str_replace("{client_id}",$arrProdDetails[$i]['client_id'],$config['files']['products']).$prodImage;
				}
				
				$outArrMyOfferDetails["offerBackImage"] = '';
				$prodBackImage = isset($arrProdDetails[$i]['offer_back_image']) ? $arrProdDetails[$i]['offer_back_image'] : "";
				if ($prodBackImage !='') {
					$outArrMyOfferDetails["offerBackImage"] = str_replace("{client_id}",$arrProdDetails[$i]['client_id'],$config['files']['products']).$prodBackImage;
				}
...
// 			$outArrMyOfferDetails["additionalOfferMedia"]=array();
	...
	public function modCheckClientGroupsPipe($pUserInfo){
		try{
			global $config;			
			$outResponse= array();
			...
			$arrClientGroups = $this->objPublic->getClientGroupTriggerIDs();
			// print_r($arrClientGroups);
			// $clientMarkersStr = '';
			// $arrClientMarkers = array();
			// for ($i=0; $i < count($arrClientGroups); $i++) { 
			// 	$cgClientGroupId = isset($arrClientGroups[$i]['id']) ? $arrClientGroups[$i]['id'] : 0;
			// 	$cgClientId = isset($arrClientGroups[$i]['client_id']) ? $arrClientGroups[$i]['client_id'] : 0;
			// 	$cgTriggerId = isset($arrClientGroups[$i]['trigger_id']) ? $arrClientGroups[$i]['trigger_id'] : 0;
			// 	$clientMarkersStr = $cgClientGroupId.'|'.$cgClientId;
			// 	$arrClientMarkers[$i] = $clientMarkersStr;
			// 	// if ($i < (count($arrClientGroups)-1)) {
			// 	// 	$clientMarkersStr .= ',';
			// 	// }
			// }
			// echo 'clientMarkersStr: '.$clientMarkersStr;
			// $clientGroupValues = isset($arrClientGroups[0]['clientGroups']) ? $arrClientGroups[0]['clientGroups'] : '';
			// $arrClientGroupValues = explode(',', $clientGroupValues);
			// $clientGroupCount = count($arrClientGroupValues);
			// // $clientGroupCount = isset($arrClientGroups) ? count($arrClientGroups) : 0;
			$outResponse['count'] = count($arrClientGroups);
			$outResponse['clientMarkers'] = $arrClientGroups;
			...
			$outResponse['customMessages']['noTracking'] = 'There is a problem with network connection. Please reset your app from your app settings';
			$outResponse['customMessages']['noTrackingOffline'] = 'Please scan atleast one catalog/image to work offline.';
			$outResponse['customMessages']['network']['title'] = 'No Network connection';
			$outResponse['customMessages']['network']['msg'] = 'Please check your wifi or cellular data network and try again';
			echo json_encode($outResponse);
		}
		catch ( Exception $e ) {
			echo 'Message: ' .$e->getMessage();
	...
	public function modCheckClientGroups($pUserInfo){
		try{
			...
			// $clientGroupCount = isset($arrClientGroups) ? count($arrClientGroups) : 0;
			$outResponse['count'] = count($arrClientGroups);
			$outResponse['clientMarkers'] = $arrClientMarkers;
			$outResponse['recentChanges'] = $arrRequiredChanges;
			$outResponse['customMessages']['noTracking'] = 'There is a problem with network connection. Please reset your app from your app settings';
			$outResponse['customMessages']['noTrackingOffline'] = 'Please scan atleast one catalog/image to work offline.';
			$outResponse['customMessages']['network']['title'] = 'No Network connection';
