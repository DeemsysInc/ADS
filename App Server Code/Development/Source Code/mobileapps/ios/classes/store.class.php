...
class cStore{
		...
		try{
		    ...
		    if (count($arrOrderInfo) > 0) {
		    	...
		    	for($c=0;$c<count($arrOrderInfo['clients']); $c++){
					...
					$offerDiscount = isset($arrOrderInfo['clients'][$c]['offer_discount']) ? $arrOrderInfo['clients'][$c]['offer_discount'] : 0;
					$offerId = isset($arrOrderInfo['clients'][$c]['offer_id']) ? $arrOrderInfo['clients'][$c]['offer_id'] : 0;
		        	...
		        	if ($orderId == 0) {
						...
						if ($insertOrder){
							//insert the offer discount order in the table order_offer
							if(isset($offerId) && $offerId!=0)
							{
								$pArray = array();
								$pTableName = 'order_offer';
								$pArray['order_id'] = $insertOrder;
								$pArray['offer_id'] = $offerId;
								$pArray['user_id'] = $orderUserId;
								$pArray['offer_discount'] = $offerDiscount;
								$pArray['created_date'] = 'NOW()';
						        $pArray['status'] = '1';
								$insertOrderOfferDetails = $this->objStore->insertQuery($pArray, $pTableName, true);
					...
					//$clientOrderIDs[$c]['offer_discount'] = $offerDiscount;
			    }
	...
	public function doOrderUpdate($pUserInfo){
		try{
			...
			for($c=0;$c<count($arrOrderInfo['clients']); $c++){
				...
				$offerDiscount = isset($arrOrderInfo['clients'][$c]['offer_discount']) ? $arrOrderInfo['clients'][$c]['offer_discount'] : '';
				$offerId = isset($arrOrderInfo['clients'][$c]['offer_id']) ? $arrOrderInfo['clients'][$c]['offer_id'] : 0;
				...
				if ($isClientExists == 1) {
					...
					if($con['order_id'] !=0){
						...
						if ($updateClientOrder) {
							...
							//update the offer discount order in the table order_offer
							if(isset($offerId) && $offerId!=0)
							{
								$uArray = array();
								$uArray['offer_id'] = $offerId;
								$uArray['offer_discount'] = $offerDiscount;
								$uArray['user_id'] =$orderUserId;
								$uArray['updated_date'] = 'NOW()';
								$con=array();
								$con['order_id'] = $clientOrderId;
								$updateOrderOfferDetails = $this->objStore->updateRecordQuery($uArray, "order_offer", $con);
							}
							for($i=0;$i< count($arrClientProds); $i++){
								$isOrderSuccess = 1;
			...
			if ($isOrderSuccess !=1) {
			 	$outArrOrder['msg']='failwerwerer';
				$outArrOrder['meher']='something'.$isOrderSuccess;
			 } else{
			 	$outArrOrder['msg']='success';
	...
	public function doUserOrderInMain($pUserInfo){
		try{
			...
			//$offerDiscount = isset($arrOrderInfo['offer_discount']) ? $arrOrderInfo['offer_discount'] : 0;
			...
			if ($orderId == 0) {
				...
				//$pArray['user_order_offer_discount'] = $offerDiscount;
				...
				//$wdArray['user_order_offer_discount'] = $offerDiscount;
	...
	public function modOrderUpdate($pUserInfo){ 
		try{
			...
			// $arrOrderProcess['orderId'] = 1;
			// $arrOrderProcess['userId'] = 9;
			// $arrOrderProcess['paymentMethod'] = 'Paypal';
			// $arrOrderProcess['paymentMethodId'] = 1;
			// $arrOrderProcess['session_id'] = 'sdfadsfasdf98989sd8f90as8df';
			...
			for ($i=0; $i < count($arrClientIds); $i++) { 
				...
				if($con['order_id'] !=0){
					...
					if ($updateClientOrder) {
						...
						/* $offerDiscount = isset($arrOrderProcess['clients'][$c]['offer_discount']) ? $arrOrderProcess['clients'][$c]['offer_discount'] : '';
						$offerId = isset($arrOrderProcess['clients'][$c]['offer_id']) ? $arrOrderProcess['clients'][$c]['offer_id'] : 0;
						//update the offer discount order in the table order_offer
						if(isset($offerId) && $offerId!=0)
						{
							$uArray = array();
