...
class mPublic extends SQLQuery {
	...
	function getClientWithRelatedProducts($clientId, $productId, $layoutType)
	{
		...
		LEFT JOIN `offers_discount_type` AS odt ON odt.client_id=of.client_id AND  odt.offer_discount_type_id =of.offer_discount_type_id AND  odt.offer_discount_status = 1
...
*/
		...
		if($layoutType!="financial"){
			$query .= " GROUP BY pd.pd_id ASC LIMIT 0,5";
		}else{
	...
	function getClientAllTriggersForXML($clientId)
	{
		$query = "SELECT tr.id AS trigger_id, tr.url AS triggerUrl, tr.client_id, tr.title AS triggerTitle, tr.height AS triggerHeight, tr.trigger_by_vertical, 
			...
			CASE WHEN `vi`.client_game_id IS NULL or `vi`.client_game_id = '' THEN 'null' ELSE `vi`.client_game_id END AS clientGameId
			...
			WHERE `tr`.`url`!='' AND `tr`.`active`=1 ";
			if($clientId != 0){ 
				$query.=" AND `tr`.`client_id` IN (".$clientId.")"; 
			}
			$query.="GROUP BY vi.id ORDER BY vi.id ASC";
		//echo $query;
		$result = $this->selectQueryForAssoc($query);
		return $result;
	}
	function getTriggersForXML($triggerId)
	{
		$query = "SELECT tr.id AS trigger_id, tr.url AS triggerUrl, tr.client_id, tr.title AS triggerTitle, tr.height AS triggerHeight, tr.trigger_by_vertical, 
			tr.width AS triggerWidth, `vi`.`url` AS visualUrl,`vi`.`id` AS visual_id,`pd`.`pd_image` AS pdImage,
			pd.`pd_name` AS `prodName`, cl.background_color, cl.light_color, cl.dark_color, cl.client_vertical_id, cl.name AS clientName, vi.id AS visualId, 
...
*/
	...
	function getClientTriggers($arrClient){
			$query = "SELECT * FROM `trigger` as tr WHERE  `tr`.`url`!='' AND `tr`.`active`=1 ";		 
			if($clientId != 0){ 
				$query.=" AND `tr`.`client_id` IN (".$arrClient.")"; 
			}
	...
	function getClientAllTriggersForJson($clientId,$triggerId)
	{
		$query = "SELECT tr.id AS trigger_id, tr.url AS triggerUrl, tr.client_id, tr.title AS triggerTitle, tr.height AS triggerHeight, tr.trigger_by_vertical, 
			tr.width AS triggerWidth, `vi`.`url` AS visualUrl,`vi`.`id` AS visual_id, `pd`.`pd_image` AS pdImage,
			pd.`pd_name` AS `prodName`, cl.background_color, cl.light_color, cl.dark_color, cl.client_vertical_id, cl.name AS clientName, vi.id AS visualId, 
	...
	function getGameById($gameId){
		$query = "SELECT * FROM `client_games_details` WHERE `client_game_id`=$gameId";
		$result = $this->selectQueryForAssoc($query);
		return $result;
