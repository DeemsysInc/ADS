...
class cPageBuilder{
	...
	public function __construct(){
		...
		require_once SRV_ROOT.'classes/shoppingcart.class.php';
		$this->objShoppingCart = new cShoppingCart();
	}
	...
	public function pageContent($pUrl){
		try{
			...
			if ($pAction=='mobileapps'){
				if (isset($pUrl[1]) && ($pUrl[1]==$config['android']['version'])){ 
					...
					if (isset($pUrl[2]) && ($pUrl[2]=='change_log')){
						$this->objPublic->modChangeLog();
					}
					if (isset($pUrl[2]) && ($pUrl[2]=='client_trigger_xml')){
						$clientIds = isset($_REQUEST['clientIds']) ? $_REQUEST['clientIds'] : (isset($pUrl[3]) ? $pUrl[3] : 0);
						$this->objPublic->modClientTriggersWithXML($pUrl, $clientIds);
					}
					if (isset($pUrl[2]) && ($pUrl[2]=='trigger_xml_details')){
						$triggerId = isset($_REQUEST['triggerId']) ? $_REQUEST['triggerId'] : (isset($pUrl[3]) ? $pUrl[3] : 0);
						$this->objPublic->modTriggersWithXML($pUrl, $triggerId);
					}
					if (isset($pUrl[2]) && ($pUrl[2]=='trigger_json')){
						$clientIds = isset($_REQUEST['clientIds']) ? $_REQUEST['clientIds'] : (isset($pUrl[3]) ? $pUrl[3] : 0);
						$this->objPublic->modClientTriggersWithJson($pUrl, $clientIds);
					}
					if (isset($pUrl[2]) && ($pUrl[2]=='client_trigger_json')){					
						 $clientIds = isset($_REQUEST['clientIds']) ? $_REQUEST['clientIds'] : (isset($pUrl[3]) ? $pUrl[3] : 0);
						 $triggerIds = isset($_REQUEST['triggerIds']) ? $_REQUEST['triggerIds'] : (isset($pUrl[4]) ? $pUrl[4] : 0);
						$this->objPublic->modClientTriggersForJson($pUrl, $clientIds,$triggerIds);
					}
					if (isset($pUrl[2]) && ($pUrl[2]=='visual_xml')){
						$visualId = isset($_REQUEST['visual_id']) ? $_REQUEST['visual_id'] : (isset($pUrl[3]) ? $pUrl[3] : 0);
						$this->objPublic->modGetVisualById($pUrl, $visualId);
					}
					if (isset($pUrl[2]) && ($pUrl[2]=='client_product')){
						...
						}else if (isset($pUrl[4]) && ($pUrl[4]=='products')){
							$this->objPublic->modProductsWithXml($pUrl);
						}else if (isset($pUrl[4]) && ($pUrl[4]=='all_products')){
							$this->objPublic->modClientAllProductsWithXml($pUrl);
						} else {
					...
					if (isset($pUrl[2]) && ($pUrl[2]=='my_offers')){
						if (isset($pUrl[3]) && ($pUrl[3]=='offer_info')){
							...
							$offerId = isset($_REQUEST['offerId']) ? $_REQUEST['offerId'] : (isset($pUrl[5]) ? $pUrl[5] : 0);
							$flag = isset($_REQUEST['flag']) ? $_REQUEST['flag'] : (isset($pUrl[3]) ? $pUrl[3] : "");
							$this->objAppMyOffers->modDeleteMyOffersWithUserId($loggedInUserId, $flag,$offerId);
						}else if(isset($pUrl[3]) && ($pUrl[3]=='redeem')){
							$loggedInUserId = isset($_REQUEST['userId']) ? $_REQUEST['userId'] : (isset($pUrl[4]) ? $pUrl[4] : 0);
							$offerId = isset($_REQUEST['offer_id']) ? $_REQUEST['offer_id'] : (isset($pUrl[5]) ? $pUrl[5] : "");
							$this->objAppMyOffers->modRedeemOffers($loggedInUserId, $offerId);
						}
						else {
					...
					if (isset($pUrl[2]) && !empty($pUrl[2]) && ($pUrl[2]=='deletewishlistproduct'))
					{
						$loggedInUserId = isset($_REQUEST['loggedInUserId']) ? $_REQUEST['loggedInUserId'] : (isset($pUrl[3]) ? $pUrl[3] : 0);
						$wishListName= isset($_REQUEST['wishListName']) ? $_REQUEST['wishListName'] : (isset($pUrl[4]) ? $pUrl[4] : "");
						$productId= isset($_REQUEST['productId']) ? $_REQUEST['productId'] : (isset($pUrl[5]) ? $pUrl[5] : "");
						$this->objAppWishList->modDelSelectedWishListProduct($loggedInUserId,$wishListName,$productId);
					}
					//}
					if (isset($pUrl[2]) && !empty($pUrl[2]) && ($pUrl[2]=='crash_log')){
						$this->objPublic->modInsertCrashLog();
					}
					if (isset($pUrl[2]) && !empty($pUrl[2]) && ($pUrl[2]=='offers')){
						$clientId= isset($_REQUEST['clientId']) ? $_REQUEST['clientId'] : (isset($pUrl[3]) ? $pUrl[3] : "");
						$this->objAppMyOffers->modClientAllOffers($pUrl);
					}
					//Shopping cart functionalities.............
					if (isset($pUrl[2]) && ($pUrl[2]=='shopingcart')){
						$this->modShopingCart($pUrl);
					}
	...
	public function modShopingCart($pUrl){
		try{
			global $config;
			// echo "modStores";
			// print_r($pUrl);
			if (isset($pUrl[3]) && ($pUrl[3]=='products')){
				if (isset($pUrl[4]) && ($pUrl[4]!='')){
					$this->objShoppingCart->modProdCartInfo($pUrl[4]);
				}
			}
			if (isset($pUrl[3]) && ($pUrl[3]=='shipaddress')){
				// echo 'ship address will come soon.';
				$handle = fopen('php://input','r');
				$jsonInput = fgets($handle);
				// Decoding JSON into an Array
				$decoded = json_decode($jsonInput,true);
				$this->objShoppingCart->modStateTaxByState($decoded);
			}
