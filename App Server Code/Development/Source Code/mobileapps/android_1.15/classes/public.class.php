...
class cPublic{
	...
	public function modRelatedProductsWithXml($pUrl){
		try{
			...
			if($clientId != ""){
				...
				if($outArrClientProduct[0]["prodId"] !="" || $outArrClientProduct[0]["offer_id"] !="")
				{
					$outClientProduct=array();
					for($i=0;$i<count($outArrClientProduct);$i++)
					{
					$outArrClientProduct[$i]["financialVideoType"] ="";
					...
					if(isset($outArrClientProduct[$i]["offer_back_image"]) && $outArrClientProduct[$i]["offer_back_image"] !="null" ){
				 		$outArrClientProduct[$i]["offer_back_image"] = "http://".$_SERVER['HTTP_HOST']."/files/clients/".$clientId."/products/".$outArrClientProduct[$i]["offer_back_image"];
					} 					
					if(isset($outArrClientProduct[$i]["tapForDetailsImgs"])) {
						...
						for($f=0;$f<count($arrForVideoPdImgs);$f++){							
							if($arrForVideoPdImgs[$f]->video!=""){
								$outArrClientProduct[$i]["financialVideoStreaming"] = $config['files']['video_streaming'].$arrForVideoPdImgs[$f]->video;
								...
								$outArrClientProduct[$i]["financialVideoType"] ="video";
							}else if($arrForVideoPdImgs[$f]->audio!=""){
								$outArrClientProduct[$i]["financialVideoStreaming"] = $config['files']['video_streaming'].$arrForVideoPdImgs[$f]->audio;
				 				$outArrClientProduct[$i]["financialVideoPdImgs"] = "http://".$_SERVER['HTTP_HOST']."/files/clients/".$clientId."/additional/".$arrForVideoPdImgs[$f]->audio;
								$outArrClientProduct[$i]["financialVideoType"] ="audio";
							}else {
								$outArrClientProduct[$i]["financialVideoPdImgs"] = "";
								$outArrClientProduct[$i]["financialVideoType"] ="";
							}
		...
		try{
			...
			for($i=0;$i<count($outArrAllTriggers);$i++)
			{
				...
				if($outArrAllTriggers[$i]["discriminator"] == "VIDEO"){
					$outArrAllTriggers[$i]["videoStreaming"] = $config['files']['video_streaming'].$outArrAllTriggers[$i]["visualUrl"];
				...
				if($outArrAllTriggers[$i]["discriminator"] == "AUDIO"){
					$outArrAllTriggers[$i]["videoStreaming"] = $config['files']['video_streaming'].$outArrAllTriggers[$i]["visualUrl"];
				 	$outArrAllTriggers[$i]["visualUrl"] = "http://".$_SERVER['HTTP_HOST']."/files/clients/".$outArrAllTriggers[$i]["client_id"]."/audios/".$outArrAllTriggers[$i]["visualUrl"];
				}
	...
	public function modClientTriggersWithXML($pUrl, $clientIds){
		try{
			...
			for($i=0;$i<count($outArrAllTriggers);$i++)
			{
				 $outArrAllTriggers[$i]["videoStreaming"] ="";
				 $outArrAllTriggers[$i]["triggerUrl"] = "http://".$_SERVER['HTTP_HOST']."/files/clients/".$outArrAllTriggers[$i]["client_id"]."/triggers/".$outArrAllTriggers[$i]["triggerUrl"];
				if($outArrAllTriggers[$i]["pdImage"]==""){
					$outArrAllTriggers[$i]["offer_image"] = "http://".$_SERVER['HTTP_HOST']."/files/clients/".$outArrAllTriggers[$i]["client_id"]."/products/".$outArrAllTriggers[$i]["offer_image"];
				}
				...
				if($outArrAllTriggers[$i]["discriminator"] == "AUDIO"){
					...
					$outArrAllTriggers[$i]['Model']=$outArrAllModel;
					   ...
					   $outArrAllGame = $this->objPublic->getGameById($outArrAllTriggers[$i]["clientGameId"]);
					   if(count($outArrAllGame) > 0){
						for($k=0;$k<count($outArrAllGame);$k++)
					    {
						
						$outArrAllGame[$k]['image']= "http://".$_SERVER['HTTP_HOST']."/files/clients/".$outArrAllTriggers[$i]["client_id"]."/games/".$outArrAllGame[$k]['game_id']."/".$outArrAllGame[$k]['image'];
						}
					   }
					$outArrAllTriggers[$i]['Games']= $outArrAllGame;
	...
	public function modTriggersWithXML($pUrl, $triggerId){
		try{
			global $config;			
			$outArrAllTriggers = array();		
			$outAllTriggers=array();
			$outArrAllModel = array();	
			$outArrAllTriggers = $this->objPublic->getTriggersForXML($triggerId);
...
*/
	...
	public function modInsertCrashLog(){
		try{
			global $config;
			$outResults = array();		
			$pCrashLogTableName = "crash_log";
			$dArray['user_id'] = isset($_REQUEST['user_id']) ? $_REQUEST['user_id'] : '';
			$dArray['session_id'] = isset($_REQUEST['session_id']) ? $_REQUEST['session_id'] : $pArray['session_id'];
			$dArray['device_type'] = isset($_REQUEST['device_type']) ? $_REQUEST['device_type'] : '';
			$dArray['os_version'] = isset($_REQUEST['os_version']) ? $_REQUEST['os_version'] : '';
			$dArray['error_message'] = isset($_REQUEST['error_message']) ? $_REQUEST['error_message'] : '';
			$dArray['error_debug'] = isset($_REQUEST['error_debug']) ? $_REQUEST['error_debug'] : '';
			$dArray['crash_date'] = 'NOW()';
			$dArray['crash_type'] = isset($_REQUEST['crash_type']) ? $_REQUEST['crash_type'] : '';
			$dArray['build_version'] = isset($_REQUEST['build_version']) ? $_REQUEST['build_version'] : '';
			$dArray['additional_info'] = isset($_REQUEST['additional_info']) ? $_REQUEST['additional_info'] : '';
			$insertedUsersDetailsResult = $this->objPublic->insertQuery($dArray, $pCrashLogTableName, true);
			if($insertedUsersDetailsResult)			
				$outResults['msg'] = "success";
			else
				$outResults['msg'] = "fail";
				
			$xml = Array2XML::createXML('resultXml', $outResults);
			echo $xml->saveXML();	
		}
		catch ( Exception $e ) {
			echo 'Message: ' .$e->getMessage();
	...
	public function modChangeLog(){
		try{
			global $config;		
			$outResults = array();
			$vArray = array();
			$outResults['resultXml'] = $this->objPublic->getChangeLog();
	...
	public function modGetVisualById($pUrl,$visualId){
		try{
			global $config;			
			$outArrAllTriggers = array();		
			$outAllTriggers=array();
			...
			$outArrAllTriggers = $this->objPublic->getVisualById($visualId);
			...
			if(count($outArrAllTriggers)>0){	
			$outAllTriggers['visual'] = $outArrAllTriggers;  
			}
			$xml = Array2XML::createXML('resultXml', $outAllTriggers);
			echo $xml->saveXML();
		}
		catch ( Exception $e ) {
			echo 'Message: ' .$e->getMessage();
	...
	public function modProductsWithXml($pUrl){
		try{
			global $config;
			$clientId = isset($pUrl[3]) ? $pUrl[3] : 0;
			$productId = isset($pUrl[5]) ? $pUrl[5] : 0;
			...
			if($clientId != ""){
				$outArrClientProduct = array();
				$outArrClientProduct = $this->objPublic->getProductsDetails($clientId, $productId, $layoutType);
				//print_r($outArrClientProduct);
				$outClientProduct=array();
				for($i=0;$i<count($outArrClientProduct);$i++)
				{
	...
	public function modClientAllProductsWithXml($pUrl){
		try{
			global $config;
			$clientId = isset($pUrl[3]) ? $pUrl[3] : 0;
			$productId = isset($pUrl[5]) ? $pUrl[5] : 0;
			...
			if($clientId != ""){
				$outArrClientProduct = array();
				$outArrClientProduct = $this->objPublic->getClientProductsDetails($clientId);
				//print_r($outArrClientProduct);
				$outClientProduct=array();
				for($i=0;$i<count($outArrClientProduct);$i++)
				{
	...
	public function modClientTriggersWithJson($pUrl, $clientIds){
		try{
	...
	public function modClientTriggersForJson($pUrl, $clientIds,$triggerIds){
		try{
			global $config;			
			$outArrAllTriggers = array();		
			$outAllTriggers=array();
			$outArrAllModel = array();	
			$outArrAllTriggers = $this->objPublic->getClientAllTriggersForJson($clientIds,$triggerIds);
