...
class mCloset extends SQLQuery {
...

	{
		...
		$query = "SELECT c.*,pd.*,cl.name AS client_name FROM closet AS c 
...

	{
		...
		$query = "SELECT pd.pd_id, pd.pd_name, pd.pd_image, pd.client_id, cl.name AS client_name, IF('wishlist_id' IS NULL, '', '0') AS wishlist_id, c.closet_selection_status, c.closet_created_date FROM closet AS c LEFT JOIN products AS pd ON pd.pd_id=c.pd_id AND pd.pd_status=1 LEFT JOIN client AS cl ON cl.id=pd.client_id AND cl.active=1 WHERE c.closet_status=1 AND c.user_id=5 ORDER BY c.closet_created_date DESC";
...

	{
		...
		$query = "SELECT pd . * , cl.name AS client_name, cl.id AS client_id FROM  products AS pd LEFT JOIN  `client` AS cl ON cl.id = pd.client_id WHERE pd.pd_id IN (".$cValsArray['productId'].")";
