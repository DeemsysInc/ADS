		...
		try{
			...
			if(count($checkUserReg)==0 && $pArray['user_username']!=""){
				...
				if ($insertedUsersDetailsResultId && $pArray['user_username']!=""){
					...
					$message = "Dear ".$pArray['user_lastname']." ".$pArray['user_firstname'].", ";

					$message .= "<p>Greetings from SeeMoreInteractive. </p>";

					$message .= "<p>Please find your login details. 

						<br>Username: ".$pArray['user_username']."

						<br>Password: ".$user_password;

					$message .= "<p>Regards,<br>SeeMoreInteractive.<br>www.seemoreinteractive.com</p>";
