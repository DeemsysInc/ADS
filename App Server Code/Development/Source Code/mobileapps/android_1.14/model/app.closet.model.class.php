...
class mCloset extends SQLQuery {
	...
	function getClosetProductsValues($cValsArray)
	{
		$query = "SELECT c.*,pd.*,cl.name AS client_name, cl.background_color, cl.light_color, cl.dark_color, cl.logo AS client_logo,cl.url AS client_url ,(SELECT group_concat(`relatedto_id` separator ',') as `related_id` FROM product_related AS pr WHERE pr.relatedfrom_id= pd.pd_id ) AS related_id,(SELECT group_concat(`relatedto_offerid` separator ',') as `related_offerid` FROM product_related AS pr WHERE pr.relatedfrom_id= pd.pd_id  AND pr.relatedto_offerid !=0) AS  `related_offerid`,cld.client_details_country_code, cs.country_name, CASE WHEN cs.country_code_char2 IS NULL or cs.country_code_char2 = '' THEN 'US' ELSE cs.country_code_char2 END AS country_code_char2, CASE WHEN cs.country_languages IS NULL or cs.country_languages = '' THEN 'en' ELSE cs.country_languages END AS country_languages, cs.currency_code
			 FROM closet AS c 
		LEFT JOIN products AS pd ON pd.pd_id = c.pd_id LEFT JOIN client AS cl ON pd.client_id = cl.id LEFT JOIN `client_details` as `cld` ON `cl`.`id` = `cld`.`client_id` AND `cld`.client_details_status=1 
