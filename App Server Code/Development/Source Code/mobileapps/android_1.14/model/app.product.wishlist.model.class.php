...
class mProductWishlist extends SQLQuery {
	...
	{
		$query = "SELECT wl.wishlist_id, wl.wishlist_name, wl.user_id, wld.wishlist_details_id, wl.wishlist_created_date, 
		wld.pd_id AS prodId, pd.client_id, pd.pd_name, pd.pd_image,pd.pd_short_description,pd.pd_description,pd.pd_url,pd.pd_price,
		IFNULL(pd.pd_istryon, '0' ) AS pd_istryon,cl.background_color,cl.name AS client_name, cl.light_color, cl.dark_color, cl.logo AS client_logo,cl.url AS client_url,
		...
		FROM `wishlist` AS wl LEFT JOIN wishlist_details AS wld ON wld.wishlist_id=wl.wishlist_id AND wld.wishlist_details_status=1 LEFT JOIN products AS pd ON pd.pd_id=wld.pd_id AND pd.pd_status=1 LEFT JOIN client AS cl ON pd.client_id = cl.id LEFT JOIN `client_details` as `cld` ON `cl`.`id` = `cld`.`client_id` AND `cld`.client_details_status=1 
		...
		$query .= " GROUP BY wld.pd_id,wld.wishlist_details_id ORDER BY wl.wishlist_id DESC, wld.wishlist_details_id ASC";
		//echo $query;		
	...
	public function getUserAllWishListProducts($userId,$wishListName)
	{
		...
		pd.client_id, pd.pd_name, pd.pd_image, pd.pd_url, pd.pd_description, pd.pd_price, IFNULL(pd.pd_istryon, '0' ) AS pd_istryon, cld.client_details_country_code, cs.country_name, CASE WHEN cs.country_code_char2 IS NULL or cs.country_code_char2 = '' THEN 'US' ELSE cs.country_code_char2 END AS country_code_char2, CASE WHEN cs.country_languages IS NULL or cs.country_languages = '' THEN 'en' ELSE cs.country_languages END AS country_languages, cs.currency_code 
		FROM `wishlist` AS wl 
		LEFT JOIN wishlist_details AS wld ON wld.wishlist_id=wl.wishlist_id AND wld.wishlist_details_status=1 
		LEFT JOIN products AS pd ON pd.pd_id=wld.pd_id AND pd.pd_status=1 LEFT JOIN client AS cl ON cl.id=pd.client_id LEFT JOIN `client_details` as `cld` ON `cl`.`id` = `cld`.`client_id` AND `cld`.client_details_status=1 LEFT JOIN `countries` as `cs` ON `cld`.`client_details_country_code` = `cs`.`country_code_char2` AND `cld`.`client_details_currency_code` = `cs`.`currency_code` 
	...
	{
		$query = "SELECT * FROM `wishlist` WHERE user_id=".$loggedInUserId." AND wishlist_status=1 ORDER BY wishlist_id DESC";
		$result = $this->selectQueryForAssoc($query);
		return $result;
	}
	...
	{
		 $query = "SELECT cl.name AS  `client_name` , wl.id,wl.shared,wli.id AS `item_id`, wl.name AS wishListName, pd.client_id,pd.price, pd.title, pd.image, pd.id AS prodId, IFNULL(pd.pd_istryon, '0' ) AS pd_istryon, cld.client_details_country_code, cs.country_name, CASE WHEN cs.country_code_char2 IS NULL or cs.country_code_char2 = '' THEN 'US' ELSE cs.country_code_char2 END AS country_code_char2, CASE WHEN cs.country_languages IS NULL or cs.country_languages = '' THEN 'en' ELSE cs.country_languages END AS country_languages, cs.currency_code FROM  `wish_list` AS wl LEFT JOIN wish_list_item AS wli ON wli.wish_list_id=wl.id LEFT JOIN product AS pd ON pd.id=wli.product_id AND pd.active=1 LEFT JOIN  `client` AS cl ON cl.id = pd.client_id LEFT JOIN `client_details` as `cld` ON `cl`.`id` = `cld`.`client_id` AND `cld`.client_details_status=1 LEFT JOIN `countries` as `cs` ON `cld`.`client_details_country_code` = `cs`.`country_code_char2` AND `cld`.`client_details_currency_code` = `cs`.`currency_code` WHERE wl.user_id !='".$userId."'  AND wl.id='".$wishListId."' AND  wl.shared = 1 GROUP BY wli.product_id ORDER BY wli.product_id DESC";
