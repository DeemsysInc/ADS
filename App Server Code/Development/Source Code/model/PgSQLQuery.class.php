...
class PgSQLQuery {
    ...
    function connect($host, $user, $pwd, $dbname, $port, $sslmode) {
		try 
		{
			$putSSLMode = "";
			if ($sslmode!=""){
				$putSSLMode = "sslmode=$sslmode";
			}
			$this->_dbHandle = pg_connect("host=$host port=$port dbname=$dbname user=$user password=$pwd $putSSLMode") or die('Could not connect: ' . pg_last_error());
			if ($this->_dbHandle != 0) {
	...
	function executeQuery($query){
		try{
			//echo $query;
			$arrResult = array();
			$result2 = pg_query($this->_dbHandle, $query) or die("Cannot execute query: $query\n");
			$i=0;
	...
	function executeQueryForAssoc($query){
		...
		{
			$arrResult = array();
			$result = pg_query($this->_dbHandle, $query) or die("Cannot execute query: $query\n");
