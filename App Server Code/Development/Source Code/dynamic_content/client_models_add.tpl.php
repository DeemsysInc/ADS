...
/*$outArrGetModelProductIds = $objClients->getClientModelsByVid($visual_id);

$arrrelId=array();
for($i=0;$i<count($outArrGetModelProductIds);$i++)
{
	$arrrelId[]=$outArrGetModelProductIds[$i]['product'];
	
}*/

?>
<script>
$("form#create_models_form").submit(function(){
   var isRequired = true;
	 jQuery("form input").removeClass("error");
	if (jQuery("#model").val()==""){
		jQuery('#frm_error_model').html('Please upload Model', {type: 'error'}).css("color", "red").show();
	 ...
	 if(jQuery("#model").val()!='')
	{
		...
		if ($.inArray(jQuery("#model").val().split('.').pop().toLowerCase(), fileExtension) == -1) {
				jQuery('#frm_error_model').html("Only '.md2','.obj' formats are allowed.", {type: 'error'}).css("color", "red").show();
				jQuery("#model").addClass("error");
				isRequired = false;
		}
	}
	if(jQuery("#texture").val()!='')
	{
		...
		if ($.inArray(jQuery("#texture").val().split('.').pop().toLowerCase(), fileExtension) == -1) {
				jQuery('#frm_error_texture').html("Only '.png' format is allowed.", {type: 'error'}).css("color", "red").show();
	...
	if(jQuery("#material").val()!='')
	{
		...
		if ($.inArray(jQuery("#material").val().split('.').pop().toLowerCase(), fileExtension) == -1) {
				jQuery('#frm_error_material').html("Only '.mtl' formats are allowed.", {type: 'error'}).css("color", "red").show();
    ...
    var formData = new FormData($(this)[0]);
        formData.append('action', "add3Dmodel");
    $.ajax({
...
<form class="block-content form" action="" method="post" name="create_models_form" id="create_models_form" enctype="multipart/form-data">
