	...
	<div class="block-border"><form class="block-content form" id="table_form" method="post" action="">
		...
		<table class="table sortable no-margin" cellspacing="0" width="100%" id="cms_clients">
			...
			<tbody>
				...
				<tr>
					<td><a href="<?php echo $config['LIVE_URL']; ?>analytics/mobile_users/offer_views/date/<?php echo isset($arrDataForGraph[$i]['dates']) ? $arrDataForGraph[$i]['dates'] : ''; ?>"><?php echo isset($arrDataForGraph[$i]['dates']) ? $arrDataForGraph[$i]['dates'] : ''; ?></a></td>
					<td><?php echo isset($arrDataForGraph[$i]['offer_views']) ? $arrDataForGraph[$i]['offer_views'] : ''; ?></td>
...
<script src="http://www.google.com/jsapi"></script>
  ...
  function drawChartOffersTrack() {
    ...
    data.addRows([
     ...
     {
         echo '["'.date('M j,Y',strtotime($arrDataForGraph[$i]['dates'])).'", '.$arrDataForGraph[$i]['offer_views'].'],';
     }
      ?>
    ]);
