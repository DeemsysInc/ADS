...
public class OptionsHowTo extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_options_how_to);
		try{
			new Common().clientLogoOrTitleWithThemeColorAndBgImgByPassingColor(
					this, "ff2600",
					"ff2600",
					"ff2600",
					Common.sessionClientLogo, "How To", "");
			...
			webview.loadUrl(getString(R.string.url_how_to));
			
			new Common().clickingOnBackButtonWithAnimation(OptionsHowTo.this, ProductList.class,"0");
	    	...
	    	imgBtnCart.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					...
					try{
						...
						Toast.makeText(getApplicationContext(), "Error: HowTo imgBtnCart onClick.", Toast.LENGTH_LONG).show();
					}
				}
			});*/
			...
			String screenName = "/web/howto/?url="+getString(R.string.url_how_to);
			String productIds = "";
	    	String offerIds = "";
			...
			Toast.makeText(getApplicationContext(), "Error: HowTo onCreate.", Toast.LENGTH_LONG).show();
			ex.printStackTrace();
			String errorMsg = className+" | onCreate |   " +ex.getMessage();
		    Common.sendCrashWithAQuery(OptionsHowTo.this,errorMsg);
		}
	}
	@Override
	...
	public void onStart() {
		...
		easyTracker.set(Fields.SCREEN_NAME, " /howto/?="+getString(R.string.url_how_to));
	...
	public void onStop() {
	 ...
	 @Override
		protected void onPause() 
		{
			try{
				super.onPause();
				Boolean appInBackgrnd = new Common().isApplicationBroughtToBackground(OptionsHowTo.this);
				if(appInBackgrnd){
		...
		protected void onResume() {
			...
			if(Common.isAppBackgrnd){
				new Common().storeChangeLogResultFromServer(OptionsHowTo.this);			
