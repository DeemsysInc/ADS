...
class cPageBuilder{
	...
	public function pageContent($pUrl){
		try{
			...
			elseif(isset($pAction[2]) && $pAction[2]=="press")
			{
				if(isset($pAction[3]) && $pAction[3]=="create")
				{
					
					include SRV_ROOT.'views/press/create_press.tpl.php';
				}
				elseif(isset($pAction[3]) && $pAction[3]=="edit")
				{
					if (isset($pAction[4]) && !empty($pAction[4])){
							$this->objModules->modEditPressReleaseForm($pAction[4]);
						}else{
			...
			elseif(isset($pAction[2]) && $pAction[2]=="news")
			{
				if(isset($pAction[3]) && $pAction[3]=="create")
				{
				...
				{
					if (isset($pAction[4]) && !empty($pAction[4])){
							$this->objModules->modEditNewsForm($pAction[4]);
						}else{
			...
			elseif(isset($pAction[2]) && $pAction[2]=="users")
			{
				if(isset($pAction[3]) && $pAction[3]=="edit")
				{
					//echo "user edit";
					if (isset($pAction[4]) && !empty($pAction[4])){
						$this->objUsers->modEditUserForm($pAction[4]);
					}
					
				}
				elseif(isset($pAction[3]) && $pAction[3]=="add")
				{
					//add new users
