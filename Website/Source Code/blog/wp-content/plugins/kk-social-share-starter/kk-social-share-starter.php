...
function sss_kk_execute() {
	...
	$twitter_button = '<a rel="nofollow" href="http://twitter.com/home?status='.str_replace(array('&#038; ', ' '), array('', '%20'), convert_smart_quotes(get_the_title())).'%20'.get_permalink().$twitter_name.'" target="_blank" title="Tweet this"><img src="'.plugins_url('tweet.png', __FILE__).'" class="sss_twitter_button" /></a> ';
	
	$fb_button = '<a rel="nofollow" href="https://www.facebook.com/sharer/sharer.php?u='.get_permalink().'" title="Share this on Facebook" target="_blank"><img src="'.plugins_url('fb-share.png', __FILE__).'" class="sss_fb_button" /></a> ';
	
	$gplus_button = '<a rel="nofollow" href="https://plus.google.com/share?url='.get_permalink().'" target="_blank" title="Share this on Google Plus"><img src="'.plugins_url('g-plus.png', __FILE__).'" class="sss_gplus_button"  /></a> ';
	
	/*$post_excerpt = substr(urlencode(get_the_title()), 0, 256);
	if(has_excerpt()) $post_excerpt = substr(urlencode(get_the_excerpt()), 0, 256);*/
	$l_in_button = '<a rel="nofollow" href="http://www.linkedin.com/shareArticle?mini=true&url='.urlencode(get_permalink()).'&title=&summary=&source='.str_replace(' ', '%20', $story_source).'" target="_blank" title="Share this on LinkedIn"><img src="'.plugins_url('l-in.png', __FILE__).'" class="sss_l_in_button" /></a> ';
	
	$f_image = wp_get_attachment_image_src(get_post_thumbnail_id(), 'single-post-thumbnail');
	$pin_button = '';
	if(is_array($f_image)) {
		$pin_button = '<a rel="nofollow" href="//www.pinterest.com/pin/create/button/?url='.get_permalink().'&media='.$f_image[0].'&description='.str_replace(array('&#038; ', ' '), array('', '%20'), convert_smart_quotes(get_the_title())).'" target="_blank" title="Share this on Pinterest"><img src="'.plugins_url('pint.png', __FILE__).'" class="sss_pin_button" /></a> ';
	}
	
	//$email_button = '<a rel="nofollow" href="mailto:?subject=Article from '.str_replace(' ', '%20', $story_source).': '.str_replace(' ', '%20', get_the_title()).'&body=Hi,%0D%0A%0D%0AI found this article on '.str_replace(' ', '%20', $story_source).', and I thought you might like it:%0D%0A%0D%0A'.get_permalink().'%0D%0A%0D%0AEnjoy!" title="Share this via Email"><img src="'.plugins_url('email.png', __FILE__).'" class="sss_email_button" /></a> ';
